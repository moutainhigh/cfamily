...
import com.cmall.familyhas.api.ApiBatchUnshelveProductKL.UnshelveProductInput;
import com.cmall.familyhas.api.ApiBatchUnshelveProductKL.UnshelveProductResult;
 ...
 *
 */
public class ApiBatchUnshelveProductKL extends RootApi<UnshelveProductResult, UnshelveProductInput> {
				...
				if(null != instream){
                    ...
                    if(productCodes.size()>0) {
                    	for (String pCode : productCodes) {
    						...
    						if(dataUpdate>0) {
    							//添加批量下架审核记录
    							Map<String,Object> querMap =DbUp.upTable("pc_productinfo").dataSqlOne("select uid from pc_productinfo where product_code=:product_code", new MDataMap("product_code",pCode));
