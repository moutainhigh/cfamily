...
public class ApiBfRejectOperate extends RootFunc {
		...
		for(int i = 0; i < sku_code_array.length; i ++) {
			...
			if(bfSku == null) {
				...
				if("1".equals(skuStatus)) {
					...
					updateMap.put("sku_status", "11");
					DbUp.upTable("pc_bf_skuinfo").update(updateMap);
					
					DbUp.upTable("pc_bf_review_log").insert("sku_code", sku_code, "sku_name", skuInfo.get("sku_name"), "operate_status", "运营总监驳回", "remark", remark, "operator", user, 
