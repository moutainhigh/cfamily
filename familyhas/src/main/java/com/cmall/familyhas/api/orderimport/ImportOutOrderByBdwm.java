 ...
 */
public class ImportOutOrderByBdwm implements Runnable {
	...
	public ImportOutOrderByBdwm(List<Map<String, Object>> list) {
		this.list = list;
	}
	...
	private static void createHjyOrder(MDataMap map) {
		...
		if (reTeslaXResult.upFlagTrue()) {
			...
			detail.put("show_price",
					BigDecimal.valueOf(Double.parseDouble(map.get("product_money").toString())).toEngineeringString());
			DbUp.upTable("oc_orderdetail").dataUpdate(detail, "sku_price,show_price", "order_code,sku_code");
		...
		String updateSql = "select zid,uid from oc_orderinfo_bdwm where order_number=:order_number and product_code=:product_code and sku_code=:sku_code and order_source=:order_source";
		Map<String, Object> orderInfo = DbUp.upTable("oc_orderinfo_bdwm").dataSqlOne(updateSql, updateParam);
		if (orderInfo != null) {
			...
			DbUp.upTable("oc_orderinfo_bdwm").update(updateMap);
		}
	}
