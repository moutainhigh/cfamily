...
public class ApiForHongbaoLucky extends RootApiForToken<HongbaoLuckyResult, HongbaoLuckyInput> {
	...
	public HongbaoLuckyResult Process(HongbaoLuckyInput inputParam, MDataMap mRequestMap) {
		...
		String sql1 = "select a.* from sc_hudong_event_hongbao_info a left join sc_hudong_event_info b on a.event_code = b.event_code"
						+ " where b.event_type_code = '449748210001' and b.event_status = '4497472700020002' and b.begin_time < now() and b.end_time > now()"
				+ "";
		...
		if(null!=dataSqlList1&&dataSqlList1.size()>0) {
			...
			result.setResultCode(0);
			result.setResultMessage("当前时间无红包雨活动");
			return  result;
		}
