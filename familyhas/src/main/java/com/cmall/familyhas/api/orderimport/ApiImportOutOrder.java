 ...
 */
public class ApiImportOutOrder extends BaseImportOrder {
						...
						if (verifyOrderNumberResult.getResultCode() == 1) {
							...
							if (result.getResultCode() == 1) {
								if (list != null && list.size() > 0) {
									list = verifyImporOrderList(list, "oc_orderinfo_import");
									if (list != null && list.size() > 0) {

										/**
										 * 添加导入数据到导入订单表
										 */
										RootResult insertOutOrderResult = insertImportData(list, "oc_orderinfo_import");
												...
												try {
													...
													for (int i = 0; i < keys.size(); i++) {
														...
														List<Map<String, Object>> importData = DbUp
																.upTable("oc_orderinfo_import").dataSqlList(
