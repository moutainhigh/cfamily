...
import com.cmall.familyhas.api.video.input.ApiGetVideoListPersonalInput;
 ...
 *
 */
public class ApiGetVideoListPersonal extends RootApiForVersion<ApiGetVideoListPersonalResult,ApiGetVideoListPersonalInput> {
	
	static final Integer pageSize = 10;
	
	public ApiGetVideoListPersonalResult Process(ApiGetVideoListPersonalInput input, MDataMap mRequestMap) {
		...
		if(StringUtils.isNotBlank(memberCode)&&memberCode.equals(input.getUserCode())){
			result.setIsOwn("Y");
		}
		...
		if(StringUtils.isNotBlank(memberCode)){
			sql = "SELECT vi.*,ms.nickname,ms.avatar,case when pd.zid is not null then 'Y' else 'N' END is_praise from lv_video_info vi LEFT JOIN membercenter.mc_member_sync ms on vi.member_code = ms.member_code LEFT JOIN lv_praise_detail pd on pd.video_code = vi.video_code AND pd.member_code = '"+memberCode+"' where vi.is_delete = 0 and vi.status = '4497471600600003' and vi.member_code = '"+input.getUserCode()+"' order by vi.create_time desc";
			if(result.getIsOwn().equals("Y")){
				sql = "SELECT vi.*,ms.nickname,ms.avatar,case when pd.zid is not null then 'Y' else 'N' END is_praise from lv_video_info vi LEFT JOIN membercenter.mc_member_sync ms on vi.member_code = ms.member_code LEFT JOIN lv_praise_detail pd on pd.video_code = vi.video_code AND pd.member_code = '"+memberCode+"' where vi.is_delete = 0 and vi.member_code = '"+input.getUserCode()+"' order by vi.create_time desc";
			}
		}else{
			sql = "SELECT vi.*,ms.nickname,ms.avatar from lv_video_info vi LEFT JOIN membercenter.mc_member_sync ms on vi.member_code = ms.member_code where vi.is_delete = 0 and vi.status = '4497471600600003' and vi.member_code = '"+input.getUserCode()+"' order by vi.create_time desc";
		}		
