...
import com.cmall.familyhas.api.ApiBatchUnshelveProduct.UnshelveProductInput;
import com.cmall.familyhas.api.ApiBatchUnshelveProduct.UnshelveProductResult;
...
public class ApiBatchUnshelveProduct extends RootApi<UnshelveProductResult, UnshelveProductInput> {
	    ...
	    if(!StringUtils.isEmpty(fileRemoteUrl)) {
			...
			try {
				...
				if(null != instream){
					...
					//第二步去重，查看数据库待审批表中有没有当前商品码处于待审批状态 ,如果有将rtnList中 对应商品去除
					String sql11 = "select product_code from productcenter.pc_product_xiajia_shenpi where shenpi_status = '0'";
					List<Map<String, Object>> dataSqlList = DbUp.upTable("pc_product_xiajia_shenpi").dataSqlList(sql11, null);
					if(null!=dataSqlList&&dataSqlList.size()>0) {
						for(Map<String, Object>  map:dataSqlList) {
							...
							if(productCodes.contains(productCode)) {
								Iterator<ImportModel> it2 = rtnList.iterator();
								while(it2.hasNext()) {
									ImportModel importModel = it2.next();
									if(productCode.equals(importModel.getProdcut_code())) {
										it2.remove();
									}
								}
							}
					...
					//String execSql = " INSERT INTO productcenter.pc_product_xiajia_shenpi(uid,product_code,product_name,create_time,update_time,min_sell_price,max_sell_price,brand_code,small_seller_code,validate_flag,commit_man)  VALUES ";
					String execSql = " INSERT INTO productcenter.pc_product_xiajia_shenpi(uid,product_code,product_name,create_time,update_time,min_sell_price,max_sell_price,brand_code,small_seller_code,validate_flag,commit_man)  VALUES(?,?,?,?,?,?,?,?,?,?,?) ";
					...
					for (ImportModel importModel : rtnList) {
						if(importModel.isFlag()) {
							batchArgs.add(new Object[]{
									...
									importModel.getProdcut_code(),
									importModel.getProduct_name(),
									DateUtil.getSysDateTimeString(),
									DateUtil.getSysDateTimeString(),
									importModel.getMin_sell_price(),
									importModel.getMax_sell_price(),
									importModel.getBrand_code(),
									importModel.getSmall_seller_code(),
									importModel.getValidate_flag(),
									create_user
							});
						} 
					}
					if(!batchArgs.isEmpty()) {
						DbUp.upTable("pc_product_xiajia_shenpi").upTemplate().getJdbcOperations().batchUpdate(execSql, batchArgs);
					}
				}
				
			} catch (Exception e) {
				result.setResultMessage("导入商品失败:程序处理发生异常！" /*+ e.getLocalizedMessage()*/);
				e.printStackTrace();
			} finally {
	 ...
	 * @param file
	 */
	private  void  batchAddFlowBussinessHistory(List<ImportModel> rtnList) {
		...
		for (ImportModel importModel : rtnList) {
			...
			insertDatamap.put("flow_code", importModel.getUid());
			insertDatamap.put("flow_type", "449715390006");
			insertDatamap.put("creator", userCode);
			insertDatamap.put("create_time", DateUtil.getSysDateTimeString());
			insertDatamap.put("flow_remark", "");
				...
				if(!StringUtils.isEmpty(productCode)) {
					...
					if(null != map && map.size() > 0) {
						...
						if("4497153900060001".equals(productStatus)) {
							...
							model.setError_message("商品未上架，不能下架");
							model.setFlag(false);
							...
							model.setUid(isEmpty(map.get("uid")) ? "" : map.get("uid").toString());
