 ...
 * @author 任宏斌
 */
public class ApiForScanCodeShop extends RootApiForVersion<ApiForScanCodeShopResult, ApiForScanCodeShopInput> {
	...
	public ApiForScanCodeShopResult Process(ApiForScanCodeShopInput inputParam, MDataMap mRequestMap) {
		...
		//先从LD取节目商品
		RsyncProductForScanCode rsyncProductForScanCode = new RsyncProductForScanCode();
		...
		if(null != rsyncResponseProductForScanCode && rsyncResponseProductForScanCode.getSuccess() 
			...
			for (FormResult formResult : formArray) {//TODO LD默认报文顺序为 本档+上一档 若顺序改变 此处需更改
				...
				if(i == 0 && "1".equals(formResult.getForm_type())){//本档商品在惠家有不可售情况
					//取惠家有配置的本档节目
					List<MDataMap> pcNowGoodsList = DbUp.upTable("pc_tv").query("good_id,form_id", null, "form_fr_date<=NOW() and form_end_date>NOW() and so_id='1000001'", null, -1, -1);
					...
					List<Map<String, Object>> pcPrevGoodsList = DbUp.upTable("pc_tv")
							.dataSqlList("SELECT good_id,form_id FROM pc_tv WHERE form_end_date=(SELECT MAX(form_end_date) FROM pc_tv WHERE form_end_date<NOW() and so_id='1000001') and so_id='1000001'",null);
