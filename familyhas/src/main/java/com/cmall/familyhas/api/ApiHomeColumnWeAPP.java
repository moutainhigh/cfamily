...
import com.cmall.familyhas.api.result.ApiHomeColumnWeAppResult;
 ...
 * 
 */
public class ApiHomeColumnWeAPP extends RootApiForVersion<ApiHomeColumnWeAppResult, ApiHomeColumnWeAppInput> {
	...
	public ApiHomeColumnWeAppResult Process(ApiHomeColumnWeAppInput inputParam, MDataMap mRequestMap) {	
		ApiHomeColumnWeAppResult result = new ApiHomeColumnWeAppResult();//返回结果
		...
		String sFields = " column_name_corlor,show_num,column_bgpic,category_codes,category_limit,prod_recommend,event_code,rule_code,product_maintenance,column_code, column_name, show_name, start_time, end_time, showmore_linktype, column_type, columns_per_row, "
		...
		MOauthInfo m = getOauthInfo();
		String sWhere = "";
		if(m==null)
		{
			sWhere = " start_time <= '" + systemTime	+ "' and end_time > '" + systemTime	+ "' and is_delete = '449746250002' "
					+ "and release_flag = '449746250001' and seller_code = '" + sellerCode + "' and view_type = '4497471600100004' and nav_code = 'SMALLAPP'" 
					+ "and (column_type = '4497471600010001' or column_type = '4497471600010011' or column_type = '4497471600010016' or column_type = '4497471600010010' or column_type = '4497471600010020' "
					+ " or column_type = '4497471600010023' or column_type = '4497471600010022' or column_type = '4497471600010008' or column_type = '4497471600010002' "
					...
					+ " or column_type = '4497471600010027' or column_type = '4497471600010029' or column_type = '4497471600010030' )";
		}
		else
		{
			MDataMap retMap = DbUp.upTable("fh_agent_member_info").one("member_code",m.getUserCode());
			if(retMap!=null)
			{
				sWhere = " start_time <= '" + systemTime	+ "' and end_time > '" + systemTime	+ "' and is_delete = '449746250002' "
						+ " and release_flag = '449746250001' and seller_code = '" + sellerCode + "' and view_type = '4497471600100004'  and nav_code = 'SMALLAPP'" 
		...
		boolean buyerShowFlag = false;
		boolean prodEvaFlag = false;
		...
		for(int i = 0; i < columnMapList.size(); i ++) {
			...
			//闪购模板获取模版背景图片字段
			if ("4497471600010011".equals(mdataMap.get("column_type")) && StringUtils.isNotBlank(mdataMap.get("column_bgpic"))) {
				picUrlList.add(mdataMap.get("column_bgpic"));
			}
			
			//一行多栏模板需要判断是否采用的只能推荐
			if("4497471600010025".equals(mdataMap.get("column_type"))&&"449748540002".equals(mdataMap.get("prod_recommend"))) {
		...
		LOOP: for(MDataMap columnMap : columnMapList) {
			...
			if("4497471600020015".equals(showmoreLinkType)&&AppVersionUtils.compareTo(appVersion,"5.6.4")<0) {//拼团大图样式时，需要判断用户版本号，如果版本号低于564,大图需要切换成列表
			...
			if (StringUtils.isNotBlank(columnBgpicUrl) && ("4497471600010011".equals(homeColumn.getColumnType()))) {
			...
			if(FENXIAO.equals(columnType)) {//分销模板
				// 分销模版只对分销用户开放
			...
			}else if(TV_LIVE.equals(columnType)) {//TV直播
				//微信小程序商城首页
...
/*					if(StringUtils.isBlank(homeColumn.getStartTime())) {
			...
			}else if(PROD_EVALUATION.equals(columnType)) {//商品评价
				if(prodEvaFlag) {
					// 查询所有商品评价模板维护的商品
					String sql = "SELECT e.* FROM familyhas.fh_apphome_evaluation e WHERE e.is_delete = '0' AND " + 
			...
			}else if(BUYER_SHOW_LIST.equals(columnType)) {//买家秀列表
				if(buyerShowFlag) {
					...
					for (Map<String, Object> map : buyerShowMapList) {
						...
						// 买家秀点赞量
						int approveCount = DbUp.upTable("nc_buyer_show_approve").count("buyer_show_or_buyer_show_eva_uid",evaluation_uid);
						String approveNum = "0";
						if(approveCount > 999) {
							approveNum = "999+";
						}else {
				...
				if(subflag) {
					...
					Loop:for(MDataMap columnContentMap : items) {
						...
						if("4497471600020015".equals(columnContentMap.get("showmore_linktype"))&&AppVersionUtils.compareTo(appVersion,"5.6.4")<0) {//拼团大图样式时，需要判断用户版本号，如果版本号低于564,大图需要切换成列表
							columnContent.setShowmoreLinktype("4497471600020009");
						}else {
			...
			if(contentList.size()>0) {
				...
				if(BANNER.equals(homeColumn.getColumnType())) {//轮播广告模板
					banner = homeColumn;
				}else if(VIDEO_LIST.equals(homeColumn.getColumnType())) {//视频模板数组
					videoList.add(homeColumn);
				}else if(TV_LIVE_H.equals(columnType)) {//TV直播横滑
					if(homeColumn.getContentList().size() > 0) {
						tvLiveH = homeColumn;
					}
				}else if(TV_LIVE.equals(columnType)) {//TV直播
					if(homeColumn.getContentList().size() > 0) {
				...
				}else if(FAST_BUY.equals(columnType)) {//闪购
					fastBuy = homeColumn;
				}else if(FAST_BUY_HH.equals(columnType)) {//闪购横滑
					fastBuyHh = homeColumn;
				}else if(SECOND_BUY.equals(columnType)) {//秒杀
					secondBuy = homeColumn;
				}else if(GROUP_BUY_SHIP.equals(columnType)) {//拼团
					groupBuyShip = homeColumn;
				}else if(ONE_ADVERTEMENT.equals(columnType)) {//一栏广告
					oneAdverList.add(homeColumn);
				}else if(RECOMMEND_BUY.equals(columnType)) {//推荐商品
					recommdList.add(homeColumn);
				}else if(THREE_COLUMN.equals(columnType)) {//三栏 
						threeTwoList.add(homeColumn);
				}else if(TWO_COLUMN.equals(columnType)) {//两栏 
						twoTwoList.add(homeColumn);
				}else if(ONE_COL_MORE_ROW.equals(columnType)) {//一栏多行 
					type25ColumnList.add(homeColumn);
				}else if(PROD_EVALUATION.equals(columnType)) {//商品评价
					prodEvaluation = homeColumn;
				}else if(FENXIAO.equals(columnType)) {//分销
					fenxiao=homeColumn;
				}
			}else {
				// 买家秀没有栏目内容
				...
				if(countMap != null) {
					...
					if(num > 0) {
						...
						if(StringUtils.isNotBlank(appVersion) && AppVersionUtils.compareTo(appVersion, "5.6.4") >= 0) {
							if(BUYER_SHOW_ENTER1.equals(homeColumn.getColumnType())) {//买家秀入口样式1
								buyerShowEnter1 = homeColumn;
							}else if(BUYER_SHOW_ENTER2.equals(homeColumn.getColumnType())) {//买家秀入口样式2
								buyerShowEnter2 = homeColumn;
							}else if(BUYER_SHOW_LIST.equals(columnType)) {// 买家秀列表
								buyerShowList = homeColumn;
							}
						}
					}
				}
		...
		result.setFenxiao(fenxiao);
		result.setBanner(banner);
		result.setFastBuy(fastBuy);
		result.setFastBuyHh(fastBuyHh);
		result.setTVLiveH(tvLiveH);
		result.setTVLive(tvLive);
		result.setVideoList(videoList);
		result.setSecondBuy(secondBuy);
		result.setGroupBuyShip(groupBuyShip);
		result.setOneAdverList(oneAdverList);
		result.setRecommdList(recommdList);
		result.setThreeTwoList(threeTwoList);
		result.setTwoTwoList(twoTwoList);
		result.setOneColMoreRowList(type25ColumnList);
		result.setProdEvaluation(prodEvaluation);
		result.setBuyerShowList(buyerShowList);
		result.setBuyerShowEnter1(buyerShowEnter1);
		result.setBuyerShowEnter2(buyerShowEnter2);
