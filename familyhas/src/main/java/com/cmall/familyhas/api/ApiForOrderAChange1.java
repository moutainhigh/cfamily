 ...
 *
 */
public class ApiForOrderAChange1 extends RootApi<ApiForOrderAChangeResult, ApiForOrderAChangeInput> {
	...
	public ApiForOrderAChangeResult Process(ApiForOrderAChangeInput inputParam,MDataMap mRequestMap) {
		...
		if("449715190014".equals(order_source)){
			...
			orderAChangeResult.setResultMessage("该订单为多彩宝订单,不允许换货!!!");
			return orderAChangeResult;
		}
...
//		MDataMap addressInfo=DbUp.upTable("oc_address_info").one("small_seller_code",orderInfo.get("small_seller_code"));
		...
		for (MDataMap orderDetail : orderDetails) {
			...
			Map<String, Object> map1=DbUp.upTable("oc_order_achange").dataSqlOne(sql, new MDataMap("order_code",order_code,"sku_code",detail.getSku_code(),"oac_status","4497477800040001","oac_type","4497477800030003"));
