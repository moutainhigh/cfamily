 ...
 *
 */
public class AddNavigationMaintain extends RootFunc{
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap){
		...
		MDataMap maddMap = mDataMap.upSubMap(WebConst.CONST_WEB_FIELD_NAME);
			...
			if("4497471600020013".equals(StringUtils.trimToEmpty(maddMap.get("showmore_linktype")))) {
				...
				minserMap.put("nav_code", nav_code);
				...
				if(StringUtils.isBlank(nav_icon)) {
					minserMap.put("showmore_linkvalue",one.get("nav_name"));
				}else {
					String nav_name = MapUtils.getString(one, "nav_name", "");
					if(StringUtils.isBlank(nav_name)) {
						minserMap.put("showmore_linkvalue",one.get("remark"));
					}else {
						minserMap.put("showmore_linkvalue",nav_name);
					}
				}
			}else {
				minserMap.put("nav_code", "");
			}
			minserMap.put("min_program_id", StringUtils.trimToEmpty(maddMap.get("min_program_id")));
			minserMap.put("is_show", maddMap.get("is_show"));
			minserMap.put("channel_limit", StringUtils.isBlank(maddMap.get("channel_limit"))?"":maddMap.get("channel_limit").toString());
			minserMap.put("channels", StringUtils.isBlank(maddMap.get("channels"))?"":maddMap.get("channels").toString());
