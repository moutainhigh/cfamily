 ...
 * date 2015-09-11
 */
public class ProductMinatoSingleFuncAdd extends RootFunc {
				...
				if (splitsValue.length > 0 && null != splitsValue
					...
					for (int i = 0; i < splitsValue.length; i++) {
						...
						if(result1>0&&result2>0){
							query = DbUp.upTable("pc_product_minatosingle").queryAll("product_code,end_time,start_time", "", "product_code=:product_code", new MDataMap("product_code",splitsValue[i]));
						...
						if(flag){
							...
							if (mResult.upFlagTrue()) {
								/** 将惠家有信息插入pc_product_minatosingle表中 */
								DbUp.upTable("pc_product_minatosingle").dataInsert(
										mAddMaps);
							}
						}
					}
