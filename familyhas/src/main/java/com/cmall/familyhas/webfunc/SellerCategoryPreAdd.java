 ...
 * 
 */
public class SellerCategoryPreAdd extends RootFunc {
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		synchronized(this){
			...
			if (mResult.upFlagTrue()) {
				Map<String, Object> codeMap = DbUp.upTable(mPage.getPageTable()).dataSqlOne("select max(category_code) from uc_sellercategory_pre  where parent_code=:parent_code and seller_code=:seller_code", mAddMaps);
				Map<String, Object> sortMap = DbUp.upTable(mPage.getPageTable()).dataSqlOne("select max(sort) from uc_sellercategory_pre where parent_code=:parent_code and seller_code=:seller_code", mAddMaps);
				...
				if(!sortMap.isEmpty()&&sortMap.get("max(sort)")!=null&&!"".equals(sortMap.get("max(sort)"))){
					...
					Map<String, Object> caMap = DbUp.upTable(mPage.getPageTable()).dataSqlOne("select sort from uc_sellercategory_pre  where category_code=:parent_code and seller_code=:seller_code", mAddMaps);
