...
public class FuncAddSellerInfo  extends RootFunc {
		...
		if (mResult.upFlagTrue()) {
			...
			for (MWebField mField : mPage.getPageFields()) {
				...
				if (mAddMaps.containsKey(mField.getFieldName())
					...
					String sValue="";
					if(mField.getFieldName().equals("small_seller_code")){
						String saller_code=mAddMaps.get(mField.getFieldName());
						sValue = saller_code.substring(0, saller_code.indexOf(","));
//						String small_seller_code=sValue.toString();
//						Map<String, String> sellermap=DbUp.upTable("oc_address_info").oneWhere("small_seller_code","","small_seller_code=:small_seller_code","small_seller_code",small_seller_code);
//						if(sellermap!=null){
//							mResult.setResultMessage(sValue+"商户售后地址已存在，请勿重复添加！");
//							return mResult;
//						}

					}else{
						sValue= mAddMaps.get(mField.getFieldName());
					}
