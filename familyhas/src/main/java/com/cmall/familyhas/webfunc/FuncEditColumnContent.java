 ...
 *
 */
public class FuncEditColumnContent extends FuncEdit {
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		if (endTime.compareTo(startTime) <= 0) {
			...
			return mResult;
		}else if (endTime.compareTo(update_time) <= 0) {
//			当前时间必须小于结束时间!
			mResult.inErrorMessage(916401214);
			return mResult;
		}else 
			if (0 < this.checkTimeRepeat(columnCode,position,startTime, endTime,uid,floorModel)) {
			//同一位置不允许时间重叠!
		...
		mDataMap.put("zw_f_showmore_linktype", mDataMap.get("zw_f_showmore_linktype").trim().replace(",", ""));
		...
		// 品牌搜索
		if("4497471600020014".equals(mDataMap.get("zw_f_showmore_linktype"))) {
			if(StringUtils.isBlank(mDataMap.get("zw_f_showmore_linkvalue"))) {
				mResult.setResultCode(0);
				mResult.setResultMessage("品牌不能为空");
				return mResult;
			}
		}
		...
		if(mDataMap.get("zw_f_column_type").equals("4497471600010017")){  //【视频播放列表】相关
			if(StringUtils.isBlank(mDataMap.get("zw_f_showmore_linkvalue"))){
		...
		if (StringUtils.isNotEmpty(floorModel)){
			...
			List<MDataMap> mapList = getContent(columnCode,startTime, endTime,floorModel,uid);
	...
	private int checkTimeRepeat(String columnCode,String position,String startTime,String endTime,String uid,String floorModel){
		String sWhere =" column_code='"+columnCode+"' and position='"+position+"' and (('"+startTime+"' between start_time and end_time) or ('"+endTime+"' between start_time and end_time) or (start_time between '"+startTime+"' and '"+endTime+"') or (end_time between '"+startTime+"' and '"+endTime+"')) and uid != '"+uid+"'  and is_delete='449746250002'";
		if (StringUtils.isNotEmpty(floorModel)){
	...
	private List<MDataMap> getContent(String columnCode,String startTime,String endTime,String floorModel,String uid){
		String sWhere =" column_code='"+columnCode+"' and (('"+startTime+"' between start_time and end_time) or ('"+endTime+"' between start_time and end_time)) and uid != '"+uid+"' and is_delete='449746250002'";
