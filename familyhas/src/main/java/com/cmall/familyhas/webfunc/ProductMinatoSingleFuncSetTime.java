 ...
 *date 2015-9-11
 */
public class ProductMinatoSingleFuncSetTime extends RootFunc {
	...
	try {
		...
		MDataMap mDataMap3 = DbUp.upTable("pc_product_minatosingle").oneWhere(
		...
		if (mDataMap3 != null) {
			...
			mDataMaps = DbUp.upTable("pc_product_minatosingle").queryAll(
			...
			if (mWebResult.upFlagTrue()) {
				...
				/** 将惠家有信息更改pc_productinfo表中 */
				DbUp.upTable("pc_product_minatosingle").update(
						mDataMap3);
			}else {
				mWebResult.inErrorMessage(916421257);
			}
