 ...
 * 新增自动选品
 */
public class FuncAddXuanpin extends FuncAdd{
	...
	public MWebResult funcDo(String sOperateUid, MDataMap mDataMap) {
		...
		if(DbUp.upTable("pc_product_xuanpin").count("name", StringUtils.trimToEmpty(mAddMaps.get("name"))) > 0) {
			result.setResultCode(0);
		...
		insertMap.put("xp_code", WebHelper.upCode("XP"));
		...
		insertMap.put("creator", UserFactory.INSTANCE.create().getRealName());
		insertMap.put("update_time", insertMap.get("create_time"));
		
		DbUp.upTable("pc_product_xuanpin").dataInsert(insertMap);
