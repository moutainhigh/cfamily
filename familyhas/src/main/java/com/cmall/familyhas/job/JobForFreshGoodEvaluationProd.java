 ...
 * @author lgx
 */
public class JobForFreshGoodEvaluationProd extends RootJob{

	@Override
	public void doExecute(JobExecutionContext context) {
		// 查看评论商品表中是否有数据,有数据才执行定时
		List<Map<String, Object>> dataSqlList = DbUp.upTable("fh_apphome_evaluation").dataSqlList("SELECT * FROM fh_apphome_evaluation", new MDataMap());
			...
			for (int i = 0; i < prodList.size(); i++) {
				...
				if(evaProd != null && evaProd.size() > 0) { // 如果有该商品
					...
					if("1".equals(is_edit)) {
						...
						DbUp.upTable("fh_apphome_evaluation").dataUpdate(mDataMap3, "location_num,evaluation_uid,order_assessment,cover_img", "product_code");
					}
				}else { // 如果没有该商品
					// 直接添加(序号,商品id,商品名称,评论uid,评价内容和第一张晒图)
					...
					Map<String, Object> fcategoryMap = DbUp.upTable("uc_sellercategory").dataSqlOne("SELECT * FROM uc_sellercategory WHERE seller_code = 'SI2003' AND category_code = '"+tcategoryMap.get("parent_code")+"'", new MDataMap());
