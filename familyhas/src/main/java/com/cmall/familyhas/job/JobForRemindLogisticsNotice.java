...
public class JobForRemindLogisticsNotice extends RootJob{
	...
	public void doExecute(JobExecutionContext context) {
		...
		String sql = "SELECT * FROM newscenter.nc_logistics_notice_push_news WHERE response_status != 1 AND push_times <= 3";//response_status 不等于 1是取推送未成功的，且推送次数少于三次的。
		pushList = DbUp.upTable("nc_logistics_notice_push_news").dataSqlList(sql,null);
		for(Map<String,Object> map : pushList) {
			if(map != null) {
				...
				DbUp.upTable("nc_logistics_notice_push_news").dataUpdate(mDataMap, "response_time,response_status,push_times,push_url", "uid");
			}
		}
	}
	...
	private String push(Map<String, Object> data,String url) {
		...
		mDataMap.put("toPage", "14");
		mDataMap.put("toUrl", data.get("order_code").toString());
