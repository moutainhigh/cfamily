...
/**
 * 福利转盘送积分失败时添加定时重新赋予积分
 * @author lgx
 *
 */
public class JobForGiveIntegralBigWheel extends RootJobForExec {

	@Override
	public IBaseResult execByInfo(String sInfo) {
		// sInfo = "MI150417100006,0.03,,,"
		...
		RootResult rootResult = plusServiceAccm.changeForAccmAmt(UpdateCustAmtInput.CurdFlag.DZ, integral, custId, "", eventCode);
		...
		if (rootResult.getResultCode() == 1) {
			...
			changeDataMap.put("change_type", "449748080013");
			changeDataMap.put("change_money", integral.toString());
			changeDataMap.put("remark", "福利转盘定时送积分");
	...
	public ConfigJobExec getConfig() {
		...
		config.setExecType("449746990022");
