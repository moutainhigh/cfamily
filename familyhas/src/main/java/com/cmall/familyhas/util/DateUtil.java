...
public class DateUtil {
	...
	/**
	 * 
	 * @return 返回yyyy-MM-dd 格式当前日期
	 */
	public synchronized static String getNoSpSysDateString2() {
		String DATETIMEONLY = "yyyy-MM-dd"; // 年-月-日
		SimpleDateFormat sysDateTimeOnly = new SimpleDateFormat(DATETIMEONLY);
		return toString(new java.util.Date(System.currentTimeMillis()), sysDateTimeOnly);
	}
	...
	public synchronized static String addMinute(int minute) {
		return DateUtil.toString(new java.util.Date(System.currentTimeMillis() + minute * DateUtil.MINUTE_MILLI),
	...
	public synchronized static String addSecond(int second) {
		return DateUtil.toString(new java.util.Date(System.currentTimeMillis() + second * DateUtil.SECOND_MILLI),
				DateUtil.sdfDateTime);
	}
	...
	/*
	 * public synchronized static void main(String[] args) { int day =
	 * DateUtil.getWeekdayOfTimestamp(new
	 * java.sql.Timestamp(System.currentTimeMillis())); //
	 * System.out.println(toString(toDate("2006-12-25")));
	 * 
	 * if (day == 1) { System.out.println("星期日"); } if (day == 2) {
	 * System.out.println("星期一"); } if (day == 3) { System.out.println("星期二"); }
	 * 
	 * // getAllWeekDays(2008,2);
	 * 
	 * // String t = "2000-02-01 01:02:00"; // String t2 = "2000-03-10 10:02:00"
...
//
//		String dateb = "2012-03-31 10:00:03";
//		java.util.Date date2 = convertToDate(dateb, DATE_FORMAT_DATETIME);
       String dataParam = "2018-07-09 10:30:00";
       java.util.Date date = toDate(dataParam,DateUtil.DATE_FORMAT_DATETIME);
       java.util.Date newDate = addDays(date, 8);
       String stringDate = toString(newDate,DateUtil.DATE_FORMAT_DATETIME);
       
       System.out.println(converSecondsToDate(86400));
	...
	public static boolean getTimefag(String now_time, String time) {
		...
		try {
			date1 = DateUtil.convertToDate(now_time, "yyyy-MM-dd HH:mm:ss");
			date2 = DateUtil.convertToDate(time, "yyyy-MM-dd HH:mm:ss");
	...
	/**
	 * @Description:计算获取两个时间 的时间差（秒数） @param startTime 开始时间 @param endTime
	 * 结束时间 @param format 格式 @author 张海生 @date 2015-8-18 下午5:30:05 @return
	 * int @throws
	 */
	public static int timeSubtraction(String startTime, String endTime, String format) {
		SimpleDateFormat d = new SimpleDateFormat(format);// 格式化时间
		long result = 0;
		try {
			result = (d.parse(endTime).getTime() - d.parse(startTime).getTime()) / 1000;// 获取秒数
		} catch (ParseException e) {
			e.printStackTrace();
		}
 ...
 */
	 ...
	 * @return
	 */
	public static String addDateSec(String day, int second){   
	        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	        java.util.Date date = null;   
	        try {   
	            date = format.parse(day);   
	...
	/**
	 * 通过Calendar型的时间数据得到月的第几号。
	 * 
	 * @param timeIsLong
	 * @return
	 */
	public synchronized static int getMonthDay() {
		Calendar calendar = Calendar.getInstance();
		return calendar.get(Calendar.DAY_OF_MONTH);
	}
	/**
	 * 日期转换
	 * @param 2020-06-17 11:18:00
	 * @return 2020年06月17日  11:18
	 */
	public static String converDateToChinese(String dateTime) {
		String converDate = "";
		if(StringUtils.isBlank(dateTime)) {
			dateTime = getNowTime();
		}
		converDate = StringUtils.substring(dateTime, 0,16).replaceFirst("-", "年").replaceFirst("-", "月").replaceFirst(" ", "日 ");
		return converDate;
	}
	/**
	 * 时间区间转换，秒数转换为小时表示
	 ...
	 * @return 01:30:00
	 */
	public static String converSecondsToDate(int seconds) {
