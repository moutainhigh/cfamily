var liveRoomProduct = {
	...
	init : function(options) {
		...
		liveRoomProduct.temp.checkdata[s.id]={};
		...
		else if(sValues)
		{
			...
			for(var i=0,j=aV.length;i<j;i++)
			{
				...
				liveRoomProduct.temp.checkdata[s.id][aV[i]]={key:aV[i],text:aT[i]};
			}
		...
		zapjs.zw.api_call('com_srnpr_zapweb_webapi_ChartApi', {
			...
			liveRoomProduct.upData(oData, s);
	...
	upData : function(oData, s) {
		...
		liveRoomProduct.temp.loadcount[s.id] =$('#' + s.id).find('table').datagrid({
			rownumbers : true,
			autoRowHeight : false,
			pagination : true,
			checkOnSelect:true,
			...
			{
				//zapjs.d(data);
				//var opts =liveRoomProduct.temp.loadcount[s.id].getChecked();
				//window.console.debug(opts);
				
				var odata=tableData.rows;
				
				var tempcheck=liveRoomProduct.temp.checkdata[s.id];
				//zapjs.d(odata);
				for(var p in odata)
				{
					var othis=odata[p];
					if(tempcheck[othis["f_0"]])
					{
						...
						liveRoomProduct.temp.loadcount[s.id].datagrid('checkRow',p);
					}
			...
			{
				liveRoomProduct.reset_checked(s.id);
			},
	...
	{
		if(liveRoomProduct.temp.loadcount[sId]!==undefined)
				{
					var checked = liveRoomProduct.temp.loadcount[sId].datagrid('getChecked');
					
					var tempcheck=liveRoomProduct.temp.checkdata[sId];
	...
	ok_full_value:function(sFncName,sParentId,sBoxId)
	{
		...
		liveRoomProduct.reset_checked(sBoxId);
		
		var tempcheck=liveRoomProduct.temp.checkdata[sBoxId];
...
if ( typeof define === "function" && define.amd) {
	define("cevent/js/liveRoomProduct", ["lib/easyui/locale/easyui-lang-zh_CN"], function() {
