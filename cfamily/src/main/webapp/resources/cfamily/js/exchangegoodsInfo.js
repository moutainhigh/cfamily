var exchangegoodsInfo = {
	...
	changeAddr : function(small_seller_code){
		...
		zapjs.f.window_box({
			id : 'changeAddr',
			content : '<iframe src="../show/page_chart_v_select_oc_address_info?zw_s_iframe_select_source=changeAddr&zw_f_small_seller_code='+small_seller_code+'&zw_s_iframe_select_page=page_chart_v_select_oc_address_info&zw_s_iframe_max_select=1&zw_s_iframe_select_callback=parent.exchangegoodsInfo.addAddr" frameborder="0" style="width:100%;height:600px;"></iframe>',
			width : '800',
			height : '650'
		});
	},
	...
	addAddr : function(sId,va, sVal){
		...
		zapjs.zw.api_call('com_cmall_familyhas_api_ApiForUpdateAfterSaleAddr1',obj,function(result) {
	...
	message:function (flag,id){
		...
		var asale_code=$("#exchange_no").val();
				...
				if('check'==flag){
					...
					$('#'+id).dialog({
						...
						buttons:[{
							text: '收货人', 
							handler: function() { 
								exchangegoodsInfo.message("owner",id);
								zapjs.f.window_close(id);
							}
						},{
							text: '下单人', 
							handler: function() { 
								exchangegoodsInfo.message("buyer",id);
								zapjs.f.window_close(id);
							}
	...
	approve:function (flag,return_code){
		...
		if(flag=="cancel"){
			txt="取消换货";
			opid="2ef8d99f10af499794f416b80e794818";
		}else{
			txt="客服确认";
			opid="f8bf480addad402eaa7f540aab9f7f0c";
		}
...
//		}
		...
		exchangegoodsInfo.message("check");
	}
	
};
if (typeof define === "function" && define.amd) {
	define("cfamily/js/exchangegoodsInfo",["zapjs/zapjs","zapjs/zapjs.zw","zapadmin/js/zapadmin_single"],function() {
