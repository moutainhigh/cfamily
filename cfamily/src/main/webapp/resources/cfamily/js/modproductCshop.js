var cfamily_modproduct = {
	...
	init_product : function(p) {
		...
		$('#zw_f_qualification_category_code').change(
				cfamily_modproduct.onCheckQualification);

		// *成本价格
		// document.getElementById("zw_f_cost_price").setAttribute("onkeyup","cfamily_modproduct.checkNum(this)");
		// *市场价格
		document.getElementById("zw_f_market_price").setAttribute("onkeyup",
				"cfamily_modproduct.checkNum(this)");
		// *商品重量
		document.getElementById("zw_f_product_weight").setAttribute("onkeyup",
				"cfamily_modproduct.checkNumWeight(this)");
		// 商品体积
		document.getElementById("zw_f_product_volume").setAttribute("onkeyup",
				"cfamily_modproduct.checkNumVolume(this)");

		// if ("SI2003" == cfamily_modproduct.product.smallSellerCode ||
		// "SI3003" == cfamily_modproduct.product.smallSellerCode) {
		// $("#zw_f_purchase_type").val("4497471600160002"); //经销
		// $("#zw_f_settlement_type").val("4497471600110002"); //特殊结算
		//			
		// }else if (cfamily_modproduct.product.smallSellerCode.indexOf('SF03')
		// == 0 && "SF03KJT" != cfamily_modproduct.product.smallSellerCode) {
		...
		// 海外购分类下的商品可以设置贸易类型
		if (p.usprList && categoryHWG != ''
				&& p.usprList[0].categoryCode.indexOf(categoryHWG) == 0) {
			$('#zw_f_product_trade_type').parents('.control-group')
					.removeClass('hide');
			$('#zw_f_product_trade_type')
					.val(
							cfamily_modproduct.product.pcProductinfoExt.productTradeType);
		}
	...
	SecondStep : function() {
		...
		$(".zab_page_default_header_title").html("商品管理 > 商品修改");
		cfamily_modproduct.show_edit();
	},

	onCheckQualification : function(evt) {
		var merchantSellerType = $("#merchant_seller_type").val();
		if (merchantSellerType == '4497478100050002'
				|| merchantSellerType == '4497478100050003') {
			// 跨境类商户不做校验
			return;
		}
		...
		$.each(cfamily_modproduct.product.qualification, function(i, data) {
			if (data.category_code == categoryCode
		...
		if (totalSize == 0) {
			...
			modalOption.content = "该品牌下的资质证书“" + names.join(',')
					+ "”已经过期，请尽快重新提交审核！";
			zapjs.f.modal(modalOption);
		} else if (names2.length > 0) { // 资质证书即将过期
			modalOption.content = "该品牌下的资质证书“" + names2.join(',')
					+ "”即将过期，请尽快重新提交审核！";
			zapjs.f.modal(modalOption);
		}
	},
	...
	move_elm : function() {
		...
		$('#cfamily_modifyproduct_insert_prop').insertAfter(
				'#zw_f_elm_description');
		$('#zw_f_elm_brand').hide();
		...
		$('#zw_f_after_sale_address_uid')
				.after(
						'<select id="zw_f_after_sale_address"><option value="">请选择</option></select>');
	...
	show_edit : function() {
		...
		// }
		zapjs.zw.api_call(
				'com_cmall_productcenter_service_ApiGetSellBrandForSpecial',
		...
		zapjs.zw.api_call(
				'com_cmall_productcenter_service_ApiGetPartAfterSaleAddress',
				obj1, cfamily_modproduct.up_after_sale_address);

		cfamily_modproduct.set_wvp();
		zapjs.e('zapjs_e_zapjs_f_ajaxsubmit_submit',
				cfamily_modproduct.beforesubmit);
		var ss = $('#zw_f_keyword').parent()[0].innerHTML;
		ss = ss.replace('zapadmin_single.show_box(\'zw_f_keyword\')',
				'cfamily_modproduct.show_box(\'zw_f_keyword\')');
		$('#zw_f_keyword').parent()[0].innerHTML = ss;
		document.getElementById("zw_f_market_price").setAttribute("onkeyup",
				"cfamily_modproduct.checkout_decimal(this)");// 市场价
		cfamily_modproduct.set_value();
		// 商品扩展信息为空时商品是有问题的,这句话不影响正常的修改功能，只是给个提示
		if (cfamily_modproduct.product.pcProductinfoExt == null
				|| cfamily_modproduct.product.pcProductinfoExt.productCode == '') {
			zapjs.f.message('商品' + cfamily_modproduct.product.productCode
					+ '扩展信息为空，无法对"虚拟销售量基数"进行修改，如果无须操作此字段请忽略此消息继续操作，请复制此消息给技术！');
			// return false;
		}
	},
	showdefault : function(trans) {
		...
		zapjs.zw.api_call(
				'com_cmall_productcenter_service_api_getFreightNameApi', uuid,
				function(result) {
					var freightName = result.tplName;
					eval('parent.zapadmin_single.result' + '("zw_f_traval","'
							+ (uuid.uid) + '","' + freightName + '")');
				});
	},
	set_value : function() {// 设置值
		...
		$('#zw_f_product_shortname').val(
				cfamily_modproduct.product.productShortname);// 商品简称
		...
		if (videoUrlShow != "") {
			$("#my-video").attr("poster",
					cfamily_modproduct.product.videoMainPic);
		...
		$('#zw_f_video_main_pic').val(cfamily_modproduct.product.videoMainPic);// 商品主图
		zapweb_upload.upload_file('zw_f_video_main_pic',
				cfamily_modproduct.product.videoMainPic);// 商品视频主图
		$('#zw_f_product_desc_video').val(
				cfamily_modproduct.product.productDescVideo);// 商品视频链接
		$('#zw_f_video_url').val(cfamily_modproduct.product.videoUrl);// 商品视频链接
		$('#zw_f_supplier_name').val(cfamily_modproduct.product.supplierName);// 供应商名称
																				// 已隐藏
		$('#zw_f_fictitious_sales').val(
				cfamily_modproduct.product.pcProductinfoExt.fictitiousSales);// 虚拟销售量
		...
		zapweb_upload.upload_file('zw_f_adpic_url',
				cfamily_modproduct.product.adPicUrl);
		...
		// '<tr><td><input class="c_text" value="' +
		// cfamily_modproduct.product.pcProductpropertyList[props]['propertyKey']
		// + '"/></td><td><textarea rows="2" class="c_value" value="' +
		// cfamily_modproduct.product.pcProductpropertyList[props]['propertyValue']
		// + '">' +
		...
		// continue;
		// }else
		// if(cfamily_modproduct.product.pcProductpropertyList[props]['propertyType']=='449736200003'){
		...
		for ( var i in cfamily_modproduct.product.pcPicList) {
			if (cfamily_modproduct.product.pcPicList[i].skuCode == ""
					|| cfamily_modproduct.product.pcPicList[i].skuCode == null) {
				if (i == 0) {
					picUrls = cfamily_modproduct.product.pcPicList[i].picUrl;
				} else {
					picUrls = picUrls + '|'
							+ cfamily_modproduct.product.pcPicList[i].picUrl;
				}
			}
		}
		...
		$('#zw_f_editor').val(
				cfamily_modproduct.product.description.descriptionInfo);// 商品描述
		// zapweb_upload.upload_file('zw_f_editor',cfamily_modproduct.product.description.descriptionInfo);
		$('#zw_f_description_pic').val(
				cfamily_modproduct.product.description.descriptionPic);// 描述图片
		zapweb_upload_modify.upload_file('zw_f_description_pic',
				cfamily_modproduct.product.description.descriptionPic);
		...
		$('#zw_f_volumn_length').val(
				cfamily_modproduct.product.productVolumeItem.split(',')[0]);// 长
		$('#zw_f_volumn_width').val(
				cfamily_modproduct.product.productVolumeItem.split(',')[1]);// 宽
		$('#zw_f_volumn_high').val(
				cfamily_modproduct.product.productVolumeItem.split(',')[2]);// 高

		$('#zw_f_purchase_type').val(
				cfamily_modproduct.product.pcProductinfoExt.purchaseType);// 采购类型
		$('#zw_f_settlement_type').val(
				cfamily_modproduct.product.pcProductinfoExt.settlementType);// 结算方式
		...
		$('#zw_f_qualification_category_code').val(
				cfamily_modproduct.product.qualificationCategoryCode);// 保质期单位

		var trans = cfamily_modproduct.product.transportTemplate;
		if (trans != null && trans != "" && trans == "0") {// 运费模板
			$("#cfamily_addproduct_travel input[name=zw_f_tra_select]:eq(0)")
					.attr("checked", 'checked');
			cfamily_modproduct.showdefault(trans);
		} else if (/^\d+$/.test(trans)) {
			$("#cfamily_addproduct_travel input[name=zw_f_tra_select]:eq(1)")
					.attr("checked", 'checked');
			...
			zapjs.zw
					.api_call(
							'com_cmall_productcenter_service_api_getFreightNameApi',
							uuid,
							function(result) {
								var freightName = result.tplName;
								$(
										"#cfamily_addproduct_travel input[name=zw_f_tra_select]:eq(2)")
										.attr("checked", 'checked');
								eval('parent.zapadmin_single.result'
										+ '("zw_f_traval","' + (uuid.uid)
										+ '","' + freightName + '")');
							});
		}
		...
		if (area != null && area != "") {
			...
			zapjs.zw.api_call(
					'com_cmall_productcenter_service_api_getTemplateNameApi',
					code, function(result) {
						var templateName = result.templateName;
						eval('parent.zapadmin_single.result'
								+ '("zw_f_area_template","' + (area) + '","'
								+ templateName + '")');
					});
		}

		$('#zw_f_sell_productcode').val(
				cfamily_modproduct.product.sellProductcode);// 货号
		$('#zw_f_flag_payway').val(cfamily_modproduct.product.flagPayway);// 是否货到付款

		if (cfamily_modproduct.product.description.keyword != null
				&& cfamily_modproduct.product.description.keyword != '') {
			var labelsStr = cfamily_modproduct.product.description.keyword
					.replace(/,/g, ' '); // 将标签之间的逗号替换为空格
			$('#zw_f_keyword').val(labelsStr);// 标签显示
		}

		$('#zw_f_pic_material_url').val(
				cfamily_modproduct.product.pcProductinfoExt.picMaterialUrl);// 图片相关素材地址
		$('#zw_f_pic_material_upload').val(
				cfamily_modproduct.product.pcProductinfoExt.picMaterialUpload);// 图片相关素材上传
		zapweb_upload.upload_file('zw_f_pic_material_upload',
				cfamily_modproduct.product.pcProductinfoExt.picMaterialUpload);
	},
	...
	show_box : function(sId) {
		...
		zapjs.f
				.window_box({
					...
					content : '<iframe src="../show/'
							+ s.source
							+ '?zw_s_iframe_select_source='
							+ sId
							+ '&zw_s_iframe_select_page='
							+ s.source
							+ '&zw_s_iframe_max_select='
							+ s.max
							+ '&zw_s_iframe_select_callback='
							+ s.callback
							+ '&category_code='
							+ cfamily_modproduct.product.category.categoryCode
	...
	beforesubmit : function() {
		...
		if (null != address && address != "") {
			 ...
			 * else{ zapjs.f.message('*售后地址：不能为空'); return false; }
			 */
		// *商品标签
		...
		;
		// 市场价格的个位不能超过10位
		if (zw_f_market_price.substring(0,
				zw_f_market_price.indexOf(".") == -1 ? zw_f_market_price.length
						: zw_f_market_price.indexOf(".")).length > 10) {
			zapjs.f.message("*市场价格：值过大，不符合规则，小数点前最长为10位");
			return false;
		}
		// 商品重量的个位不能超过10位
		if (zw_f_product_weight
				.substring(
						0,
						zw_f_product_weight.indexOf(".") == -1 ? zw_f_product_weight.length
								: zw_f_product_weight.indexOf(".")).length > 10) {
			zapjs.f.message("*商品重量：值过大，不符合规则，小数点前最长为10位");
			return false;
		}

		// 商品体积的个位不能超过10位
		if (zw_f_product_volume
				.substring(
						0,
						zw_f_product_volume.indexOf(".") == -1 ? zw_f_product_volume.length
								: zw_f_product_volume.indexOf(".")).length > 10) {
		...
		// };
		// }
		if ($('#zw_f_product_desc_video').val().length > 0
				&& $('#zw_f_video_main_pic').val().length <= 0) {
		...
		// }
		if (bFlag) {
			if (!zapjs.zw.validate_field('#zw_f_description_pic',
					'469923180002', '描述图片')) {
				bFlag = false;
			}
		}
		...
		// 如果贸易类型被展示处理则必须选择一个值
		if ($('#zw_f_product_trade_type').parents('.control-group').eq(0)
				.hasClass('hide') == false) {
	...
	recheck_product : function() {
		...
		thisproduct.afterSaleAddressUid = $('#zw_f_after_sale_address_uid')
				.val();// 售后地址
		...
		thisproduct.qualificationCategoryCode = $(
				'#zw_f_qualification_category_code').val();// 保质期单位
		...
		// 商品分类属性
		thisproduct.pprList = [];
		$('#cfamily_addproduct_properties table tbody tr')
				.each(
						function(n, el) {
							var is_must = $(el).find('.zw_f_ppr_is_must').val();// 是否必填
							var properties_value_type = $(el).find(
									'.zw_f_ppr_properties_value_type').val();// 固定值/自定义
							var properties_value = $(el).find(
									'.ppr_properties_value_code').val();
							var properties_code = $(el).find(
									'.ppr_properties_value_code').attr("id")
									.split("+")[1];
							...
							if (properties_code
									&& (properties_value || properties_value_code)) {
								thisproduct.pprList
										.push({
		...
		thisproduct.pcProductinfoExt.fictitiousSales = $(
				'#zw_f_fictitious_sales').val();
		thisproduct.pcProductinfoExt.settlementType = $('#zw_f_settlement_type')
				.val();
		thisproduct.pcProductinfoExt.purchaseType = $('#zw_f_purchase_type')
				.val();
		thisproduct.pcProductinfoExt.picMaterialUrl = $(
				'#zw_f_pic_material_url').val();
		thisproduct.pcProductinfoExt.picMaterialUpload = $(
				'#zw_f_pic_material_upload').val();
		thisproduct.pcProductinfoExt.productTradeType = $(
	...
	up_prop_list : function() {
		zapjs.zw
				.api_call(
						...
						{
							categoryCode : cfamily_modproduct.product.category.categoryCode
						}, cfamily_modproduct.prop_list_success);
	},
	// 设置商品描述信息
	set_editor_info : function() {
		zapjs.zw.editor_show('zw_f_editor', $('#cfamily_addproduct_uploadurl')
				.val());
	},
	// 处理重量、体积、货到付款
	set_wvp : function() {
		...
		if (cfamily_modproduct.product.sellerCode != ''
				&& cfamily_modproduct.product.sellerCode.length != 5) {
			$('#zw_f_flag_payway').parent().parent().hide();
		}
		$('#zw_f_product_volume')
				.parent()
				.append(
						'<label id=volumn22 >(单位：立方厘米&nbsp;&nbsp;&nbsp;&nbsp;备注：含外包装)<label/>');
		$('#zw_f_product_volume').parent().parent().append(ckg);
		$('#zw_f_product_weight')
				.parent()
				.append(
						'<label id=weight22 >(单位:千克&nbsp;&nbsp;&nbsp;&nbsp;备注：含外包装)<label/>');

		document.getElementById("zw_f_product_volume").setAttribute("onkeyup",
				"cfamily_modproduct.checkNum(this)");// 体积
		document.getElementById("zw_f_volumn_length").setAttribute("onkeyup",
				"cfamily_modproduct.checkNumLimit3(this)");
		document.getElementById("zw_f_volumn_width").setAttribute("onkeyup",
				"cfamily_modproduct.checkNumLimit3(this)");
		document.getElementById("zw_f_volumn_high").setAttribute("onkeyup",
				"cfamily_modproduct.checkNumLimit3(this)");
		document.getElementById("zw_f_product_volume").setAttribute("onblur",
				"cfamily_modproduct.checkBlur(this)");
		document.getElementById("zw_f_volumn_length").setAttribute("onblur",
				"cfamily_modproduct.checkBlur(this)");
		document.getElementById("zw_f_volumn_width").setAttribute("onblur",
				"cfamily_modproduct.checkBlur(this)");
		document.getElementById("zw_f_volumn_high").setAttribute("onblur",
				"cfamily_modproduct.checkBlur(this)");
		// 商品保质期
		document.getElementById("zw_f_expiry_date").setAttribute("onkeyup",
				"cfamily_modproduct.checkout_number(this)");
	},
	// 校验只能输入数字和两位小数
	checkNum : function(ol) {
		if (ol.value == ol.value2)
			return;
		if (ol.value.search(/^\d*(?:\d\.\d{0,2})?$/) == -1)
			ol.value = (ol.value2) ? ol.value2 : '';
		else
			ol.value2 = ol.value;
	},
	...
	checkNumLimit3 : function(ol) {
		if (ol.value == ol.value2)
			return;
		if (ol.value.search(/^\d{0,3}(?:\d\.\d{0,2})?$/) == -1)
			ol.value = (ol.value2) ? ol.value2 : '';
		else
			ol.value2 = ol.value;
	},
	...
	checkNumVolume : function(ol) {
		if (ol.value == ol.value2)
			return;
		if (ol.value.search(/^\d*(?:\d\.\d{0,6})?$/) == -1)
			ol.value = (ol.value2) ? ol.value2 : '';
		else
			ol.value2 = ol.value;
	},
	...
	checkNumWeight : function(ol) {
		if (ol.value == ol.value2)
			return;
		if (ol.value.search(/^\d*(?:\d\.\d{0,3})?$/) == -1)
			ol.value = (ol.value2) ? ol.value2 : '';
		else
			ol.value2 = ol.value;
	},
	...
	get_vloumn : function() {
		...
		var width = $('#zw_f_volumn_width').val().replace(/^\s+|\s+$/g, '');
		var high = $('#zw_f_volumn_high').val().replace(/^\s+|\s+$/g, '');
		if (length != "" && width != "" && high != "" && !isNaN(length)
				&& !isNaN(width) && !isNaN(high)) {
			...
			if (length != "0" && width != "0" && high != "0") {
				if (reg.test(vlm.toString())) {
					...
					$('#zw_f_product_volume').val(
							cfamily_modproduct.volumeMultiplication(length,
									width, high));
				} else {
	...
	volumeMultiplication : function(arg1, arg2, arg3) {
		var m = 0, s1 = arg1.toString(), s2 = arg2.toString(), s3 = arg3
				.toString();
		try {
			m += s1.split(".")[1].length;
		} catch (e) {
		...
		return Number(s1.replace(".", "")) * Number(s2.replace(".", ""))
				* Number(s3.replace(".", "")) / Math.pow(10, m);
	},
	// 读取属性列表数据
	...
	up_brand_info : function(oResult) {
		...
		for ( var i in oList) {
			$('#zw_f_brand').append(
					'<option value="' + oList[i]["brandCode"] + '">'
							+ oList[i]["brandName"] + '</option>');
		}
	...
	up_after_sale_address : function(oResult) {
		var oList = oResult.list;
		for ( var i in oList) {
			$('#zw_f_after_sale_address').append(
					'<option value="' + oList[i]["uid"] + '">'
							+ oList[i]["afterSaleAddresName"] + '</option>');
		}
		$('#zw_f_after_sale_address').val(
				cfamily_modproduct.product.afterSaleAddressUid);
	...
	new_show_prop : function() {
		...
		for ( var s in cfamily_modproduct.product.productSkuInfoList) {
			...
			for ( var skv in skl.skuValue.split('&')) {
				alCheckCode.push(skl.skuKey.split('&')[skv].split('=')[1] + '&'
						+ skl.skuValue.split('&')[skv].split('=')[1] + '&'
						+ skl.skuPicUrl);
			}
		}
		...
		for ( var p in aSale) {
			...
			aExtendProp.push('<tr><td>' + ofather["property_name"]
					+ '</td><td>');
			if (oSub.length > 0) {
				aExtendProp.push('<select id="cfamily_addproduct_extend_'
						+ aSale[p] + '">');
				aExtendProp.push('<option value=""></option>');
				for ( var i in oSub) {
					aExtendProp.push('<option value="'
							+ oSub[i]["property_code"] + '">'
							+ oSub[i]["property_name"] + '</option>');
				}
		...
		if (aExtendProp.length > 0) {
			$('#cfamily_addproduct_pextend table tbody').prepend(
					aExtendProp.join(''));
		...
		for ( var props in cfamily_modproduct.product.pcProductpropertyList) {
			if (cfamily_modproduct.product.pcProductpropertyList[props]['propertyType'] == '449736200004') {
				...
				$('#cfamily_addproduct_custom table tbody')
						.append(
								'<tr><td><input class="c_text" value="'
										+ cfamily_modproduct.product.pcProductpropertyList[props]['propertyKey']
										+ '"/></td><td><textarea rows="2" class="c_value" value="'
				...
				$(
						'#cfamily_addproduct_extend_'
								+ cfamily_modproduct.product.pcProductpropertyList[props]['propertyKeycode'])
						.val(
								cfamily_modproduct.product.pcProductpropertyList[props]['propertyCode']);
				continue;
			}
		}
	},
...
if (typeof define === "function" && define.amd) {
	define("cfamily/js/modproductCshop", [ "zapweb/js/zapweb_upload",
			"zapweb/js/zapweb_upload_modify", "zapjs/zapjs.comboboxc",
			"zapadmin/js/zapadmin_single" ], function() {
