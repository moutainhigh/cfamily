var addColumnApp = {
	init : function() {
		...
		zapjs.e('zapjs_e_zapjs_f_ajaxsubmit_submit', addColumnApp.beforeSubmit);
		addColumnApp.showMoreInit();
		addColumnApp.noticeInit();
		// 链接类型字段把显示浮层类型去掉
		$("#zw_f_showmore_linktype option[value='4497471600020005']").remove();
		$('#zw_f_product_maintenance').change(function(){
			addColumnApp.show_releated_field();
		});
		//$('#zw_f_prod_recommend').change(function(){
		//	addColumnApp.show_category_limit();
		//});
		$('#zw_f_category_limit').change(function(){
			addColumnApp.show_category();
		});
	...
	showMoreInit : function() {
		...
		if (columnType == "4497471600010004") { // 导航栏
		...
		} else if (columnType == "4497471600010017") { // 视频播放列表 是否显示更多 需要隐藏
			...
			if(columnType == "4497471600010019") {
				...
				$("#zw_f_show_name").parent().parent().hide();
				$("#zw_f_is_showmore").parent().parent().hide();
			}else if(columnType == "4497471600010037"){
				$("#zw_f_is_showmore").val("449746250002");
				$("#zw_f_is_showmore").parent().parent().hide();
	...
	showMoreSelect : function() {
		...
		if (showmore == 449746250001) {// 是
			if (columnType == "4497471600010010"
					|| columnType == "4497471600010016" || columnType == "4497471600010011" || columnType == "4497471600010026" || columnType == "4497471600010028" || columnType == "4497471600010036" || columnType == "4497471600010037") {// 如果栏目类型为TV直播
...
if (typeof define === "function" && define.amd) {
	define("cfamily/js/addColumnApp", function() {
		return addColumnApp;
	});
}
// 切换是否显示更多时使用
