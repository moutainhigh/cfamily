...
cfamily_flowbussiness = {
		...
		callChangeFlow : function(flowType,flowbussinessid){
			...
			$.getJSON(url, function(data){
				...
				if("已下架" == data.resultMessage.substring(174, 177)){
					showHtml += "<form class='form-horizontal' id='flow_form_forsubmit' method='POST' style='margin-top:20px;'><input type='hidden' id='zw_f_to_status' name='zw_f_to_status' value=''>" +
						"<input type='hidden' id='zw_f_flow_bussinessid' name='zw_f_flow_bussinessid' value='"+flowbussinessid+"'><input type='hidden' id='zw_f_flow_type' name='zw_f_flow_type' value='"+flowType+"'>" +
								"<div class='control-group'><label class='control-label' for='zw_f_source_code'>备注信息:</label><div class='controls'><textarea id='zw_f_remark' name='zw_f_remark'></textarea></div></div>";
					...
					showHtml += "<div><div>商品已下架</div></div>"; 
				}
...
if ( typeof define === "function" && define.amd) {
	define("cfamily/js/flowbussinessForProduct_ro", [], function() {
