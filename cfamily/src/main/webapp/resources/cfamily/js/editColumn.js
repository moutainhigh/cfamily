var editColumn = {
	init : function() {
		//$("#zw_f_view_type").val("4497471600100001");
		...
		zapjs.e('zapjs_e_zapjs_f_ajaxsubmit_submit', editColumn.beforeSubmit);
		editColumn.showMoreInit();
		editColumn.noticeInit();
		// 链接类型字段把显示浮层类型去掉
		$("#zw_f_showmore_linktype option[value='4497471600020005']").remove();
		$('#zw_f_product_maintenance').change(function(){
			editColumn.show_releated_field();
		});
		//$('#zw_f_prod_recommend').change(function(){
		//	editColumn.show_category_limit();
		//});
		$('#zw_f_category_limit').change(function(){
			editColumn.show_category();
		});
	...
	show_releated_field :function (reset){
		// 先重置
...
if (typeof define === "function" && define.amd) {
	define("cfamily/js/editColumn", function() {
		return editColumn;
	});
}
// 切换是否显示更多时使用
