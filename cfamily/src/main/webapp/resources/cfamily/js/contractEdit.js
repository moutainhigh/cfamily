var contractEdit = {
		init : function(){
			var ct = $("#zw_f_contract_type").val();
			$("#zw_f_contract_type").prepend('<option value="" selected>请选择</option>');
			$("#zw_f_contract_type").val(ct);
			$("#zw_f_end_time").parent().parent().after('<div id="addPro" class="controls"><input class="btn btn-success" type="button" value="添加商品" onclick="contractEdit.show_windows()"/></div>');
			var opt ={};
			opt.productCode = $("#zw_f_product_code").val();
			zapjs.zw.api_call('com_cmall_productcenter_service_api_ApiGetProductSimpleInfor', opt, contractEdit.up_product_info);
			$("#zw_f_dissolution_time").parent().parent().find("label").prepend('<span class="w_regex_need">*</span>');
			contractEdit.contractTypeChange();
			...
			zapjs.e('zapjs_e_zapjs_f_ajaxsubmit_submit',contractEdit.beforeSubmit);
		},
	...
	up_product_info : function(oResult) {
		...
		if(typeof(pi.product_code) == "undefined"){
			return;
		}
	...
	beforeSubmit : function() {
		...
		var contractType = $("#zw_f_contract_type").val();
		if(contractType == ""){
			zapjs.f.message("合同类型不能为空！");
			return false;
		}
...
if (typeof define === "function" && define.amd) {
	define("cfamily/js/contractEdit", ["zapjs/zapjs","zapjs/zapjs.zw","zapadmin/js/zapadmin_single"],function() {
		return contractEdit;
	});
}
