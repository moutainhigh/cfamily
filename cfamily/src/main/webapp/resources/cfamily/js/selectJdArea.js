var selectJdArea = {
	...
	bindChangeEvent: function() {
		if(this.config.provinceId) {
			$('#'+this.config.provinceId).change((function(level){
				return function(){
					selectJdArea.onChange(level, this);
				}
			})('province'));
		}
	...
	loadSelect: function(level, $target, pCode, selectedValue){
		...
		zapjs.zw.api_call('com_cmall_ordercenter_service_api_ApiForGetJdAddress',param, (function(){
			return function(data){
				...
				if(data.addressList){
					...
					for(var i in data.addressList) {
						$target.append('<option value="'+data.addressList[i].code+'">'+data.addressList[i].name+'</option>');
					}
					...
					if(data.addressList.length == 0 && level == 'town') {
						$target.hide();
					} else {
						$target.show();
					}
...
if (typeof define === "function" && define.amd) {
	define("cfamily/js/selectJdArea",function() {
		return selectJdArea;
	});
}