var xuanpinEdit = {
	...
	init : function() {
		...
		if($('#zw_f_category_limit').val() == "4497471600070002") {
			this.plusData.categoryCodes = $('#zw_f_category_codes').val().split(',')
		}
		
		if($('#zw_f_brand_limit').val() == "4497471600070002") {
			this.plusData.brandCodes = $('#zw_f_brand_codes').val().split(',')
		}
		
		if($('#zw_f_uc_seller_type_limit').val() == "4497471600070002") {
			var codes = $('#zw_f_uc_seller_type_codes').val().split(',');
			for(var i in codes) {
				$('input[name="zw_f_uc_seller_type_codes_checkbox"][value="'+codes[i]+'"]').prop('checked', true)
			}
		}		
		
		if($('#zw_f_small_seller_limit').val() == "4497471600070002") {
			var codes = $('#zw_f_small_seller_codes').val().split(',');
			for(var i in codes) {
				$('input[name="zw_f_small_seller_codes_checkbox"][value="'+codes[i]+'"]').prop('checked', true)
			}
		}
		
		if($('#zw_f_sell_price_limit').val() == "4497471600070002") {
			var rangeVal = $('#zw_f_sell_price_range').val();
			var vals = rangeVal.split('-');
			if(vals.length == 2) {
				$('#zw_f_sell_price_range_start').val(vals[0]);
				$('#zw_f_sell_price_range_end').val(vals[1]);
			}
		}
		
		if($('#zw_f_product_stock_limit').val() == "4497471600070002") {
			var rangeVal = $('#zw_f_product_stock_range').val();
			var vals = rangeVal.split('-');
			if(vals.length == 2) {
				$('#zw_f_product_stock_range_start').val(vals[0]);
				$('#zw_f_product_stock_range_end').val(vals[1]);
			}
		}
		
		$('#zw_f_product_status_limit').val($('#zw_f_product_status_limit_hidden').val());
		
		$('#zw_f_category_limit').change();
		$('#zw_f_uc_seller_type_limit').change();
		$('#zw_f_small_seller_limit').change();
		$('#zw_f_sell_price_limit').change();
		$('#zw_f_product_stock_limit').change();
		$('#zw_f_brand_limit').change();
		$('#zw_f_comment_limit').change();
		$('#zw_f_comment_good_limit').change();
		$('#zw_f_rebuy_rate_limit').change();
	...
	showChooseCategoryBox: function(){
		zapjs.f.window_box({
			...
			content : '<iframe src="../show/page_chart_choose_category_cf?zw_f_category_code='+xuanpinEdit.plusData.categoryCodes.join(',')+'&zw_s_iframe_choose_callback=parent.xuanpinEdit.chooseCategoryCompleted&zw_f_maxLevel=3" frameborder="0" style="width:100%;height:500px;"></iframe>',
			width : '780',
			height : '580'
		});			
	},
	chooseCategoryCompleted: function(data) {
		...
		xuanpinEdit.plusData.categoryCodes = objList
		xuanpinEdit.refreshCategoryTable();
	},	
	//刷新分类列表
	refreshCategoryTable : function(){
		var obj = {};
		obj.categoryCodeArr = xuanpinEdit.plusData.categoryCodes;
			...
			$('#categoryTable').datagrid({
			    ...
			    columns:[[
						...
						{field:'opt', title:'操作',width:50,align:'center', formatter:function(value,rec,index){  
							return '<a href="javascript:void(0)" mce_href="#" onclick="xuanpinEdit.delTableRow(\'categoryTable\',\''+ index +'\')">删除</a> ';  
		                }}
			    ]],
			    title:'分类列表',
			    width:750,
	...
	showChooseBrandBox: function(){
		zapjs.f.window_box({
			...
			content : '<iframe src="../show/page_chart_choose_item_cf?zw_s_iframe_page_code=page_chart_v_pc_brandinfo_for_select&zw_s_iframe_choosed_values='+xuanpinEdit.plusData.brandCodes.join(',')+'&zw_s_iframe_choose_callback=parent.xuanpinEdit.chooseBrandCompleted" frameborder="0" style="width:100%;height:500px;"></iframe>',
			width : '780',
			height : '550'
		});			
	},	
	chooseBrandCompleted: function(data) {
		var objList = jQuery.parseJSON(data);
		xuanpinEdit.plusData.brandCodes = objList
		xuanpinEdit.refreshBrandTable();
	},	
	//刷新品牌列表
	refreshBrandTable : function(){
		var obj = {};
		obj.brandCodeArr = xuanpinEdit.plusData.brandCodes.join(',');
		...
		zapjs.zw.api_call('com_cmall_familyhas_api_ApiForCouponLimitBrandBaseInfo',obj,function(result) {
			...
			$('#brandTable').datagrid({
			    ...
			    columns:[[
						...
						{field:'opt', title:'操作',width:50,align:'center', formatter:function(value,rec,index){  
							return '<a href="javascript:void(0)" mce_href="#" onclick="xuanpinEdit.delTableRow(\'brandTable\',\''+ index +'\')">删除</a> ';  
		                }}
			    ]],
			    title:'品牌列表',
			    width:750,
	...
	beforeSubmit: function() {
		...
		if($('#zw_f_category_limit').val() == "4497471600070002") {
			var codes = xuanpinEdit.plusData.categoryCodes.join(',');
			if(codes == '') {
		...
		if($('#zw_f_brand_limit').val() == "4497471600070002") {
			var codes = xuanpinEdit.plusData.brandCodes.join(',');
			if(codes == '') {
...
if (typeof define === "function" && define.amd) {
	define("cfamily/js/xuanpinEdit", function() {
		return xuanpinEdit;
	});
}