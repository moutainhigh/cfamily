var merchantEditDrfat = {

		init:function(value){
			//zapjs.e('zapjs_e_zapjs_f_ajaxsubmit_submit',merchantEditDrfat.beforeSubmit);
			...
			$('#zw_f_account_clear_type').change(function(){
				merchantEditDrfat.change_account_clear_type();
			});
			
			$("#zw_f_bank_name").select2(); 
			$("#s2id_zw_f_bank_name").attr("style","width:220px");
			merchantEditDrfat.redStarAdd();
			...
			$("#zw_f_business_license_type").change(merchantEditDrfat.selectBusinessLicense);
		...
		approveSubmit : function() {
			...
			if(zw_f_organization_code.attr("zapweb_attr_regex_id")!="" && $.trim(zw_f_organization_code.val()).length==0){
				zapjs.f.message('组织机构代码影印件必须上传');
				return false;
			}
			var zw_f_organization_number = $("#zw_f_organization_number");
			if(zw_f_organization_number.attr("zapweb_attr_regex_id")!="" && zw_f_organization_number.val().length == 0){
			...
			if(zw_f_organization_number_validity_period.attr("zapweb_attr_regex_id")!="" && $.trim(zw_f_organization_number_validity_period.val()).length==0){
			...
			if(zw_f_organization_number_validity_period_end.attr("zapweb_attr_regex_id")!="" && $.trim(zw_f_organization_number_validity_period_end.val()).length==0){
				zapjs.f.message('组织机构代码有效期结束不能为空');
				return false;
			}
			var zw_f_tax_registration_certificate_copy = $("#zw_f_tax_registration_certificate_copy");
			if(zw_f_tax_registration_certificate_copy.attr("zapweb_attr_regex_id")!="" && $.trim(zw_f_tax_registration_certificate_copy.val()).length==0) {
				zapjs.f.message('税务登记证副本必须上传');
				return false;
			}
			var zw_f_taxpayer_certificate_pic = $("#zw_f_taxpayer_certificate_pic");
			if(zw_f_taxpayer_certificate_pic.attr("zapweb_attr_regex_id")!="" && $.trim(zw_f_taxpayer_certificate_pic.val()).length==0){
				zapjs.f.message('一般纳税人资格证电子版必须上传');
				return false;
			}
			var zw_f_tax_identification_number = $("#zw_f_tax_identification_number");
			if(zw_f_tax_identification_number.attr("zapweb_attr_regex_id")!="") {
		...
		selectProvince: function(obj) {
			...
			zapjs.zw.api_call('com_cmall_systemcenter_service_ChinaAreaService', {'code':code, "area":"city"
				...
				merchantEditDrfat.selectCity(_code);
			});
		},
		selectCity: function(code) {
			zapjs.zw.api_call('com_cmall_systemcenter_service_ChinaAreaService', {'code':code, "area":"county"
				...
				merchantEditDrfat.selectCounty();
			});
		},
...
if (typeof define === "function" && define.amd) {
	define("cfamily/js/merchantEditDrfat",function() {
		return merchantEditDrfat;
	});
}