/*
 * zapweb_upload_modify
 * 文件上传类
 */

var zapweb_upload_modify = {
	...
	upload_file : function(sFieldName, sUploadUrl) {
		...
		zapweb_upload_modify.upload_show(sFieldName);
	...
	upload_result : function(o) {
		// zapjs.f.setdomain();
		parent.zapweb_upload_modify.upload_success(o, zapjs.f.urlget('zw_s_source'));
	},
	// 上传展示
		...
		if ($('#' + sField).val() != "") {
			...
			if ($('#' + sField).val() != "" && sFiles.length > 0) {
				...
				for (var i = 0, j = sFiles.length; i < j; i++) {
					...
					var sShowHex = zapweb_upload_modify.is_image(sFix) ? ('<img src="'
							+ sFiles[i] + '" />') : sFix;
					...
					if (sUploadView == "12") {
						aHtml
								.push('<li class="control-upload-image-li"><div class="control-upload-image-div"><a href="'
										...
										+ '" target="_blank">'
										+ sShowHex
										+ '</a></div><div class="control-upload-delete"><span class="btn btn-mini " onclick="zapweb_upload_modify.upload_delete(\''
										+ sField
										+ '\','
										+ i
										...
										+ '" target="_blank">'
										+ sShowHex
										+ '</a></div><div class="control-upload-delete"><span class="btn btn-mini " onclick="zapweb_upload_modify.upload_modify(\''
										+ sField
										+ '\','
										+ i
						...
						aHtml
								.push('<div class="w_left w_p_10">'
										+ (j > 1 ? '<a href="javascript:zapweb_upload_modify.change_index(\''
												+ sField
												+ '\','
												+ i
												+ ',\'up\')">上移</a><a href="javascript:zapweb_upload_modify.change_index(\''
												+ sField
												+ '\','
												+ i
												+ ',\'down\')">下移</a>'
												: '')
										+ '<a href="javascript:zapweb_upload_modify.change_index(\''
										+ sField + '\',' + i
										+ ',\'delete\')">删除</a>'
										+ '<a href="javascript:zapweb_upload_modify.change_index(\''
										+ sField + '\',' + i
						...
						if (sUploadTitle != "") {
							...
							aHtml.push('<div>标题信息：<input id="' + sUploadTitle
									+ '_zapweb_upload_modify_title_' + i
		...
		if (sUploadLink != "") {
			...
			for (var i = 0, j = sFiles.length; i < j; i++) {
				var sLinkUrl = zapjs.f.formatsplit($(
						'#' + sUploadLink + '_zapweb_upload_modify_link_' + i).val());
		...
		if (sUploadTitle != "") {
			...
			for (var i = 0, j = sFiles.length; i < j; i++) {
				var sTitleUrl = zapjs.f
						.formatsplit($(
								'#' + sUploadTitle + '_zapweb_upload_modify_title_'
										+ i).val());
	...
	change_index : function(sField, iIndex, sLink) {
		...
		var sUploadLink = zapjs.f.upset(sSetParams, 'zw_s_link');
		if (sUploadLink != "") {
			zapweb_upload_modify.save_link(sField);
		...
		var sUploadTitle = zapjs.f.upset(sSetParams, 'zw_s_title');
		if (sUploadTitle != "") {
			zapweb_upload_modify.save_title(sField);
		...
		if (iIndex == -99) {
			...
			sFiles = zapweb_upload_modify.array_change(sFiles, iIndex, iIndex - 1);
			if (sUploadLink != "") {
				sLinks = zapweb_upload_modify.array_change(sLinks, iIndex, iIndex - 1);
			}
			if (sUploadTitle != "") {
				sTitles = zapweb_upload_modify.array_change(sTitles, iIndex,
						iIndex - 1);
			}
		} else if (sLink == "down") {
			...
			$("#zw_page_upload_iframe_zw_f_description_pic").contents().find("#file").click();
			$("#cfamily_upload_iframe_zw_f_description_pic").val(iIndex+'_true');
		}
		...
		zapweb_upload_modify.upload_show(sField);
	...
	upload_delete : function(sField, iIndex) {
		 ...
		 * zapweb_upload_modify.upload_show(sField);
		 */
		zapweb_upload_modify.change_index(sField, iIndex, 'delete');

	},
	// 修改文件
	upload_modify : function(sField, iIndex) {
		zapweb_upload_modify.change_index(sField, iIndex, 'modify');
	},
	// 上传成功
	upload_success : function(o, sField) {
		...
		// alert($("#cfamily_upload_iframe_zw_f_description_pic").val());
		if (o.resultCode == 1) {
			if (o.resultObject) {
				/*
				 ...
				 * zapweb_upload_modify.upload_show(sField);
				 */
				var iIndex = $("#cfamily_upload_iframe_zw_f_description_pic").val().split('_')[0];
				var flag = $("#cfamily_upload_iframe_zw_f_description_pic").val().split('_')[1];
				if("true" == flag){
					// 如果是多文件上传 则替换第一个文件
					if (o.resultList != null && o.resultList.length > 1) {
						zapweb_upload_modify.upload_replace(sField, iIndex, o.resultList[0]);
					} else {
						zapweb_upload_modify.upload_replace(sField, iIndex, o.resultObject);
					}
				} else {
					// 如果是多文件上传 则开始插入多个文件
					if (o.resultList != null && o.resultList.length > 1) {
						...
						zapweb_upload_modify.change_index(sField, -99, o.resultObject);
					}
				}
				
				if (zapjs.f.callextend("zapjs_e_zapweb_upload_modify_upload_success")) {

				}
				
				zapweb_upload_modify.saveSize(o.sizeMap);
			}
		} else {
			alert(o.resultMessage);
		}
		$("#cfamily_upload_iframe_zw_f_description_pic").val('-1_false');
	},
	// 上传提交
	...
	saveSize : function(info) {
		for(var key in info) {
			zapweb_upload_modify.imgSizeMap[key] = info[key];
		}  
	},
	//存放图片的大小（单位：B）
	imgSizeMap : {}
...
if (typeof define === "function" && define.amd) {
	define("zapweb/js/zapweb_upload_modify", function() {
		return zapweb_upload_modify;
	});
}
