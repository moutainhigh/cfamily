/*
 * zapweb_upload_appendix
 * 文件上传类
 */

var zapweb_upload_appendix = {
	...
	upload_file : function(sFieldName, sUploadUrl) {
		...
		zapweb_upload_appendix.upload_show(sFieldName);
	...
	upload_result : function(o) {
		// zapjs.f.setdomain();
		parent.zapweb_upload_appendix.upload_success(o, zapjs.f.urlget('zw_s_source'));
	},
	// 上传展示
	upload_show : function(sField) {  //sfield:zw_f_appendix
...
*/
		...
		if ($('#' + sField).val() != "") {
			...
			if ($('#' + sField).val() != "" && sFiles.length > 0) {
				...
				for (var i = 0, j = sFiles.length; i < j; i++) {
					...
					var realFname = sFiles[i].split('_');

					var sFix = aFname[aFname.length - 1];
					
					var reaFname = realFname[0];

					
					/*var sShowHex = zapweb_upload_appendix.is_image(sFix) ? ('<img src="'
							+ realFname[1] + '"  download="'+realFname[1]+'"/>') : reaFname+"."+sFix;*/
							
					var sShowHex = reaFname+"."+sFix;
					if (sUploadView == "12") {
						aHtml
								.push('<li class="control-upload-image-li"><div class="control-upload-image-div"><a href="'
										...
										+ '" target="_blank">'
										+ sShowHex
										+ '</a></div><div class="control-upload-delete"><span class="btn btn-mini " onclick="zapweb_upload_appendix.upload_delete(\''
										+ sField
										+ '\','
										+ i
										+ ')"><i class="icon-trash "></i>&nbsp;&nbsp;删除</span><a href="'
										+ realFname[1]
										+ '" target="_blank">'
										+ sShowHex
										+ '</a></div><div class="control-upload-delete"><span class="btn btn-mini " onclick="zapweb_upload_appendix.upload_modify(\''
										+ sField
										+ '\','
										+ i
										+ ')"><i class="icon-trash "></i>&nbsp;&nbsp;修改</span></li>');
					} else {
						//alert(sShowHex)
						var sView = '';
						...
						aHtml.push('<div class="w_left w_w_100"><a   href="'
								+ realFname[1] + '"  download target="_blank">' + sShowHex
								+ '</a></div>');

						aHtml
								.push('<div class="w_left w_p_10">'
										+ '<a href="javascript:zapweb_upload_appendix.change_index(\''
										+ sField + '\',' + i
										+ ',\'delete\')">删除</a></div>');
						...
						if (sUploadLink != "") {
							...
							aHtml.push('<div>链接地址：<input id="' + sUploadLink
									+ '_zapweb_upload_appendix_link_' + i
						...
						if (sUploadTitle != "") {
							...
							aHtml.push('<div>标题信息：<input id="' + sUploadTitle
									+ '_zapweb_upload_appendix_title_' + i
		...
		if (sUploadLink != "") {
			...
			for (var i = 0, j = sFiles.length; i < j; i++) {
				var sLinkUrl = zapjs.f.formatsplit($(
						'#' + sUploadLink + '_zapweb_upload_appendix_link_' + i).val());
		...
		if (sUploadTitle != "") {
			...
			for (var i = 0, j = sFiles.length; i < j; i++) {
				var sTitleUrl = zapjs.f
						.formatsplit($(
								'#' + sUploadTitle + '_zapweb_upload_appendix_title_'
										+ i).val());
	...
	change_index : function(sField, iIndex, sLink) {
		...
		var sUploadLink = zapjs.f.upset(sSetParams, 'zw_s_link');
		if (sUploadLink != "") {
			zapweb_upload_appendix.save_link(sField);
		...
		var sUploadTitle = zapjs.f.upset(sSetParams, 'zw_s_title');
		if (sUploadTitle != "") {
			zapweb_upload_appendix.save_title(sField);
		...
		if (iIndex == -99) {
			...
			sFiles = zapweb_upload_appendix.array_change(sFiles, iIndex, iIndex - 1);
			if (sUploadLink != "") {
				sLinks = zapweb_upload_appendix.array_change(sLinks, iIndex, iIndex - 1);
			}
			if (sUploadTitle != "") {
				sTitles = zapweb_upload_appendix.array_change(sTitles, iIndex,
						iIndex - 1);
			}
		} else if (sLink == "down") {
		...
		} else if (sLink == "modify") {
			$("#cfamily_upload_iframe_zw_f_appendix").contents().find("#file").click();
			$("#cfamily_upload_iframe_zw_f_appendix").val(iIndex+'_true');
		}
		...
		zapweb_upload_appendix.upload_show(sField);
	...
	upload_delete : function(sField, iIndex) {
		 ...
		 * zapweb_upload_appendix.upload_show(sField);
		 */
		zapweb_upload_appendix.change_index(sField, iIndex, 'delete');

	},
	// 修改文件
	upload_modify : function(sField, iIndex) {
		zapweb_upload_appendix.change_index(sField, iIndex, 'modify');
	},
	// 上传成功
	upload_success : function(o, sField) {
		...
		// alert($("#cfamily_upload_iframe_zw_f_appendix").val());
		if (o.resultCode == 1) {
			if (o.resultObject) {
				/*
				 ...
				 * zapweb_upload_appendix.upload_show(sField);
				 */
				var iIndex = $("#cfamily_upload_iframe_zw_f_appendix").val().split('_')[0];
				var flag = $("#cfamily_upload_iframe_zw_f_appendix").val().split('_')[1];
				if("true" == flag){
					// 如果是多文件上传 则替换第一个文件
					if (o.resultList != null && o.resultList.length > 1) {
						zapweb_upload_appendix.upload_replace(sField, iIndex, o.resultList[0]);
					} else {
						zapweb_upload_appendix.upload_replace(sField, iIndex, o.resultObject);
					}
				} else {
					// 如果是多文件上传 则开始插入多个文件
					if (o.resultList != null && o.resultList.length > 1) {
						...
						zapweb_upload_appendix.change_index(sField, -99, o.resultObject);
					}
				}
				
				if (zapjs.f.callextend("zapjs_e_zapweb_upload_appendix_upload_success")) {

				}
				
				zapweb_upload_appendix.saveSize(o.sizeMap);
			}
		} else {
			alert(o.resultMessage);
		}
		$("#cfamily_upload_iframe_zw_f_appendix").val('-1_false');
	},
	// 上传提交
	...
	saveSize : function(info) {
		for(var key in info) {
			zapweb_upload_appendix.imgSizeMap[key] = info[key];
		}  
	},
	//存放图片的大小（单位：B）
	imgSizeMap : {}
...
if (typeof define === "function" && define.amd) {
	define("zapweb/js/zapweb_upload_appendix", function() {
		return zapweb_upload_appendix;
	});
}
