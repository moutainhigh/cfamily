...
var g_type_cart = {
    ...
    SendToCloud: function (callback) {
        if (g_type_cart.GetVersion() == g_type_cart.Version.V2) {
            ...
            g_type_cart.api_input = { goodsList: g_type_cart.api_input.goodsList, version: 1.0, "channelId": g_const_ChannelID };
        }
...
/*divID1:显示的层，divID2:隐藏的层，beginnum：显示第几个（从0开始）*/
    ...
    if (type == "zp") {
        ...
        for (var i = 0; i < bigpic_zp.length; i++) {
            if (bigpic_zp[i].skuCode == skuCode) {
                temp = bigpic_zp[i].imgStr;
                break;
            }
        }
    }
        ...
        for (var i = 0; i < bigpic_pj.length; i++) {
            if (bigpic_pj[i].skuCode == skuCode) {
                temp = bigpic_pj[i].imgStr;
                break;
            }
        }
    }
	...
	swiper_zp = null;
    swiper_zp = new Swiper('.swiper-container', {
        loop: false,
        pagination: '.swiper-pagination',
        nextButton: '.swiper-button-next',
        prevButton: '.swiper-button-prev',
