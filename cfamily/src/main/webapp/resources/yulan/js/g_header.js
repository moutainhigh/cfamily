...
/*接口外部地址*/var g_APIUTL_Member = "/JYH/API_Member.aspx";
...
function ShowMesaageTime(sMessage,showtime) {
    new Toast({ time: showtime, context: $('body'), message: sMessage, top: '50%' }).show();
}
...
/*根据链接类型转换链接地址,t:‘显示更多’链接类型(g_const_showmoreLinktype),u:目标, 5.3.0 增加达观推荐dgRequestId*/
var g_GetLocationByShowmoreLinktype = function (t, u, saveurl, dgRequestId) {
    ...
    if (u.Trim().length > 0) {
        switch (t) {
            case g_const_showmoreLinktype.URL:
                sreturn = u;
                if (u.toLowerCase().indexOf("/product_detail.html") > -1) {
                    //5.3.0 增加达观推荐id 开始
                    if (typeof (dgRequestId) != "undefined") {
                        localStorage["dg_dgRequestId"] = dgRequestId;
                    }
                    else {
                    ...
                    //兼容小程序
                    var tt_pid = u.split('?')[1].split('&');
                    for (var ikj = 0; ikj < tt_pid.length; ikj++) {
                        if (tt_pid[ikj].indexOf('pid') > -1) {
                            if (typeof (saveurl) != "undefined") {
                                sreturn = 'onclick=\'GoProductDetail(' + tt_pid[ikj].split('=')[1] + ',' + saveurl + ')\'';
                            }
                            else {
                                sreturn = 'onclick=\'GoProductDetail(' + tt_pid[ikj].split('=')[1] + ')\'';
                            }
                            break;
                        }
                    }
                    ...
                    /*5.1.8 首页配置跳转积分商城链接，先判断登录状态，未登录先登录 回跳使用商品详情页返回的逻辑实现跳转积分商城  开始*/
                    if (u.indexOf("jfshop=") > -1) {
                        sreturn = 'onclick=\'GoToJFShop("' + u + '")\'';
                    }
                    else {
                        if (sreturn.indexOf("?") != -1)
                            sreturn = 'href=\'' + u + '&_r=' + _r + '\'';
                        else
                            sreturn = 'href=\'' + u + '?_r=' + _r + '\'';
                    }
                    /*5.1.8 首页配置跳转积分商城链接，先判断登录状态，未登录先登录 结束*/
                }
                break;
            case g_const_showmoreLinktype.ProductDetail:
                ...
                if (typeof (saveurl) != "undefined") {
                    ...
                    sreturn = 'onclick=\'GoProductDetail(' + u + ',' + saveurl + ',null,' + dgRequestId + ')\'';
                }
                else {
                    sreturn = 'onclick=\'GoProductDetail(' + u + ',false,null,' + dgRequestId + ')\'';
                }
                break;
            case g_const_showmoreLinktype.ProductType:
                sreturn = 'href=\'/Product_List.html?_r=' + _r + '&showtype=category&keyword=' + u + '\'';
            case g_const_showmoreLinktype.KeyWordSearch:
                sreturn = 'href=\'/Product_List.html?_r=' + _r + '&keyword=' + encodeURIComponent(u) + '\'';
                break;
                //396 首页H5弹层
            case g_const_showmoreLinktype.IndexH5:
                sreturn = 'onclick=\'Action396H5.ShowH5()\'';
                break;
        ...
        sreturn = 'href=\'javascript:void(0);\'';
    return sreturn;
}

/*5.1.8 跳转积分商城*/
var GoToJFShop = function (url) {
    if (UserLogin.LoginStatus == g_const_YesOrNo.NO) {
        PageUrlConfig.SetUrl(url);
        var backurl = url;//"/product_detail.html?jfshop=1&backurl=" + escape(u);
        if (backurl.indexOf("?") > -1) {
            backurl = backurl + "&gotojfshop=1"
        }
        else {
            backurl = backurl + "?gotojfshop=1"
        }
        UserRELogin.login(backurl + "&t=" + Math.random());
        //location.href = backurl + "&t=" + Math.random();
        return
    }
    else {
        GetToken(url);
...
//        cache: false,
//        method: g_APIMethod,
//        data: "t=" + Math.random() + "&action=GetToken",
...
//            if (backurl.indexOf("?") > -1) {
//                backurl = backurl + "&token=" + msg.resultmessage;
//            }
//            else {
//                backurl = backurl + "?token=" + msg.resultmessage;
//            }
//            //location.replace(backurl + "&t=" + Math.random());
//            location.href = backurl + "&t=" + Math.random();
//        }
//    });
...
function g_GetPictrue(picurl,defaultpic) {
    if (picurl == "") {
        if (typeof (defaultpic) != "undefined")
            return defaultpic;
        else
            return g_goods_Pic;
    }
    else if (picurl == undefined) {
        if (typeof (defaultpic) != "undefined")
            return defaultpic;
        else
            return "";
...
var PageUrlConfig = {
    ...
    //传入步骤
    //传入步骤
    BackTo: function (num, delcurrpage) {
        if (delcurrpage==undefined) {
            delcurrpage = true;
        }
        var pageurl = "/index.html";
        if (localStorage.getItem(g_const_localStorage.PagePathList) != null) {
        ...
        if (g_const_PagePathList.length > 0) {
            ...
            pageurl = g_const_PagePathList[g_const_PagePathList.length - num];
                ...
                //返回页面==当前页面时，重新获取前一页面  开始
                var _dqurl = document.URL.split('?')[0];
                var _dqurlgroup = _dqurl.split('/');
                var _dqpagename = _dqurlgroup[_dqurlgroup.length - 1];
                if (typeof (pageurl) != "undefined") {
                    while (pageurl.indexOf(_dqpagename) > -1  && _dqpagename != "Product_Detail.html") {
                        for (var i = 0; i < num; i++) {
                            g_const_PagePathList.pop();
                        }
                        num = 1;
                        if (g_const_PagePathList.length >= num) {
                            pageurl = g_const_PagePathList[g_const_PagePathList.length - num];
                        }
                        else {
                            pageurl = "/index.html";
            ...
            if (delcurrpage == false) {
                g_const_PagePathList.push(document.URL);
            }
    ...
    // location = pageurl;
        ...
        return pageurl == "" ? "/index.html" : pageurl;
    },
    //传入地址
    Clear: function () {
...
var WxInfo = {
    //url:授权成功后的跳转地址
    GetPayID: function (url, fromPage) {
        /*5.2.2 优化后退机制  开始*/
        if (fromPage != null && typeof (fromPage) != "undefined") {
            
            if (fromPage.toLowerCase().indexOf(g_const_PageURL.AddressEdit.toLowerCase()) > -1 && fromPage.toLowerCase().indexOf("login=0") > -1)
            {
                var backurl = "";
                if (typeof (localStorage["tempreferrer"]) != "undefined" && localStorage["tempreferrer"] != "") {
                    backurl = localStorage["tempreferrer"];
                    localStorage["tempreferrer"] = "";
                    if (backurl.indexOf('?') > -1) {
                        window.location.href = backurl + "&t=" + Math.random();
                    }
                    else {
                        window.location.href = backurl + "?t=" + Math.random();
                    }
        ...
        if (typeof (localStorage["towxgate"]) == "undefined" || localStorage["towxgate"] != document.location.href) {
            ...
            //拼接授权失败的回调地址
            var g_const_domain_tt = g_const_domain;
            if (window.location.href.indexOf("https://") > -1) {
                g_const_domain_tt = g_const_domain_tt.replace("http://", "https://")
            }
            else if (window.location.href.indexOf("http://") > -1) {
                g_const_domain_tt = g_const_domain_tt.replace("https://", "http://")
            }

            var failurl = g_const_domain_tt + g_const_PageURL.OauthErr.substring(1, g_const_PageURL.OauthErr.length);

            //来源预授权登录标志(用于区分微信授权不同意时点后退的操作，避免死循环)
            localStorage["towxgate"] = document.location.href;//document.referrer.toLowerCase();

            //授权权限(b为基本(不返回unionID),f为完整(需用户授权，会返回unionID))
            //window.location.replace(g_const_PageURL.OauthLogin + "?oauthtype=WeiXin&returnurl=" + encodeURIComponent(backurl) + "&scope=b&ispay=1");
            window.location.replace(g_const_PageURL.OauthLogin + "?oauthtype=WeiXin&returnurl=" + encodeURIComponent(backurl + "~" + failurl) + "&scope=f&ispay=1&protocol=" + document.location.protocol);
        }
        else {
            //拒绝授权后回跳来源,5.2.2 拒绝授权后，跳转至登录，使用上号密码登录
            //window.location.replace(PageUrlConfig.BackTo());
        }
    },
    GetPayIDAct: function (url) {
...
function IsIphoneX() {
    ...
    //document.write(u);
    //document.write("<br/>");
    //document.write(/iphone/gi.test(navigator.userAgent));
    //document.write("<br/>screen.height:");
    //document.write(screen.height);
    //document.write("<br/>screen.width");
    //document.write(screen.width);
    //if (u.indexOf('iPhone') > -1 || u.indexOf('Mac') > -1 || u.indexOf('iPad') > -1 || !!u.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/) && (screen.height == 812 && screen.width == 375)) {
...
$(document).ready(function () {
   ...
   // Merchant1.RecordCheck(Merchant1.RecordPage);
    if (typeof (UserLogin) != "undefined") {
       // UserLogin.Check(AccountAutoLogin);
    }

    //
});

var AccountAutoLogin = function () {
    if (UserLogin.LoginStatus == g_const_YesOrNo.YES) {
        return;
    }
    if (localStorage[g_const_localStorage.Member] == null) {
        return;
    }
...
//        cache: false,
//        method: g_APIMethod,
//        data: "t=" + Math.random() + "&action=autologin&memberinfo=" + localStorage[g_const_localStorage.Member].toString(),
//        dataType: g_APIResponseDataType
//    });
//
//    request.done(function (msg) {
...
//                g_type_loginjs.Execute(msg.resultmessage);
//            }
//            else {
//
//            }
...
//                            }
//                            if (web_from == "panshi2016") {
//                                localStorage[g_const_localStorage.OrderFromParam] = GetQueryString("track_code");
//                            }
//                            if (web_from == "smzdm2017") {
//                                localStorage[g_const_localStorage.OrderFromParam] = GetQueryString("feedback");
//                            }
//                            //多麦处理--结束
//                            $('.mask,.guide-app').hide();
//                        }
...
//                    //}
//                    try {
//                        if (location.search.indexOf("IC") == -1) {
//                            $(".app").show();
//                            $("#app_smg").hide()
//                        }
...
//                                "&paramlist=" + escape(web_search.replace(/&/g, "@").replace(/=/g, "^").replace(/:/g, "^")) +
...
/*清空本地缓存方法（特殊处理）*/
function wxshop_LocalStorage_Clear() {
    var temp_wy_guid = "";
    //买点缓存不清除
    if (typeof (localStorage["mywxshopguid"]) != "undefined") {
        temp_wy_guid = localStorage["mywxshopguid"];
    }

    localStorage.clear();
    //重新赋值
    localStorage["mywxshopguid"] = temp_wy_guid;
}
