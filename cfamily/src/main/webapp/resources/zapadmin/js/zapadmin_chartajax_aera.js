var zapadmin_chartajax_aera = {
	...
	init : function(options) {
		...
		zapadmin_chartajax_aera.temp.options=s;
		zapadmin_chartajax_aera.temp.checkdata[s.id]={};
		...
		else if(sValues)
		{
			...
			for(var i=0,j=aV.length;i<j;i++)
			{
				...
				zapadmin_chartajax_aera.temp.checkdata[s.id][aV[i]]={key:aV[i],text:aT[i]};
			}
		...
		zapjs.zw.api_call('com_srnpr_zapweb_webapi_ChartApi', {
			...
			zapadmin_chartajax_aera.upData(oData, s);
	...
	upData : function(oData, s) {
		...
		zapadmin_chartajax_aera.temp.loadcount[s.id] =$('#' + s.id).find('table').datagrid({
			rownumbers : true,
			autoRowHeight : false,
			pagination : true,
			checkOnSelect:true,
			...
			{
				//zapjs.d(data);
				//var opts =zapadmin_chartajax_aera.temp.loadcount[s.id].getChecked();
				//window.console.debug(opts);
				
				var odata=tableData.rows;
				
				var tempcheck=zapadmin_chartajax_aera.temp.checkdata[s.id];
				//zapjs.d(odata);
				for(var p in odata)
				{
					var othis=odata[p];
					if(tempcheck[othis["f_0"]])
					{
						...
						zapadmin_chartajax_aera.temp.loadcount[s.id].datagrid('checkRow',p);
					}
			...
			{
				zapadmin_chartajax_aera.reset_checked(s.id);
			},
	...
	{
		if(zapadmin_chartajax_aera.temp.loadcount[sId]!==undefined)
				{
					var checked = zapadmin_chartajax_aera.temp.loadcount[sId].datagrid('getChecked');
					
					var tempcheck=zapadmin_chartajax_aera.temp.checkdata[sId];
	...
	ok_value:function(sFncName,sParentId,sBoxId)
	{
		...
		zapadmin_chartajax_aera.reset_checked(sBoxId);
		
		var tempcheck=zapadmin_chartajax_aera.temp.checkdata[sBoxId];
		...
		//此处为解决eaysui单选分页的bug
		var maxnum = zapadmin_chartajax_aera.temp.options["maxnum"];
		if(maxnum>0){
...
if ( typeof define === "function" && define.amd) {
	define("zapadmin/js/zapadmin_chartajax_aera", ["lib/easyui/locale/easyui-lang-zh_CN"], function() {
		return zapadmin_chartajax_aera;
	});
}
