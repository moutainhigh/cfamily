...
$(function(){
	...
	function pushClickId(code,type){
		var obj = {};
		obj.operation="set";
		obj.type = "string";
		obj.key = userkey_spfb_wzbj;
		...
		if(type == "质检员待审批"){
			obj.preKey="sbfb_zjy";
		}else if(type == "普通运营待审批"){
			obj.preKey="sbfb_ptyy";
		}else if(type == "跨境运营待审批"){
			obj.preKey="sbfb_kjyy";
		}
		zapjs.zw.api_call('com_cmall_systemcenter_api_ApiRedisOperation',obj,function(result) {
			if(result.success==true){
			}
	...
	function previewClick(code,type){
	    if(clickedIds.indexOf(code) < 0){
	    	pushClickId(code,type);
	...
	function batchApprove(){
		...
		$('input[id^="chb-item-"]').each(function(){
			if($(this).prop('checked') && $(this).attr('id') != 'chb-item-all'){
				var currentStatusStr = $(this).data('currentstatus');
				if(currentStatusStr == "质检员待审批"){
					toStatus = "4497172300160002";
					currentStatus = "4497172300160001";
					type="sbfb_zjy";
				}else if(currentStatusStr == "普通运营待审批"){
					toStatus = "4497172300160003";
					currentStatus = "4497172300160002";
					type="sbfb_ptyy";
				}else if(currentStatusStr == "跨境运营待审批"){
					toStatus = "4497172300160003";
					currentStatus = "4497172300160004";
					type="sbfb_kjyy";
				}
				checkClickIds(type);//检查缓存
		...
		showHtml = showHtml.replace('#zw_f_current_status#',currentStatus);
		showHtml = showHtml.replace('#zw_f_to_status#',toStatus);
...
<table  class="table  table-condensed table-striped table-bordered table-hover">
   ...
   <tbody>
		<#list e_pagedata.getPageData() as e_list>
			...
			<#if e_list[6]="普通运营待审批">
			<td><input id="chb-item-${e_list[0]}" type="checkbox" value="${e_list[0]}" onclick="$('#chb-item-all').prop('checked',false)" data-productcode="${e_list[3]}" data-currentstatus="${e_list[6]}"></td>
			<#elseif e_list[6]="质检员待审批">
			<td><input id="chb-item-${e_list[0]}" type="checkbox" value="${e_list[0]}" onclick="$('#chb-item-all').prop('checked',false)" data-productcode="${e_list[3]}" data-currentstatus="${e_list[6]}"></td>
			<#elseif e_list[6]="跨境运营待审批">
			<td><input id="chb-item-${e_list[0]}" type="checkbox" value="${e_list[0]}" onclick="$('#chb-item-all').prop('checked',false)" data-productcode="${e_list[3]}" data-currentstatus="${e_list[6]}"></td>
			<#else>
	  		 	...
	  		 	<#if e_index = 7	>
	  		 		...
	  		 		<td>
	  		 			<#if e?default("") = "">
	  		 				无内容
	  		 			<#else>
	  		 				<a target="_blank" href="${e?default("")}" onclick="previewClick('${e_list[0]}','${e_list[6]}')">预览</a>
	  		 			</#if>
...
<script type="text/template" id="flow_form_forsubmit_temp">
	...
	<input type="hidden" id="zw_f_to_status" name="zw_f_to_status" value="#zw_f_to_status#">
	<input type="hidden" id="zw_f_flow_code" name="zw_f_flow_code" value="#zw_f_flow_code#">
	<input type="hidden" id="zw_f_current_status" name="zw_f_current_status" value="#zw_f_current_status#">
