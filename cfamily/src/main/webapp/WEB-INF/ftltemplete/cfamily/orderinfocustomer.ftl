...
<#assign collageService=b_method.upClass("com.cmall.familyhas.service.CollageStatusService")>
<#assign orderCollageStatus = collageService.getCollageStatus(a_order_code)>
<#assign orderCollageStatusCode = collageService.getCollageStatusCode(a_order_code)> 
<div class="zab_info_page">


<div style="padding-left:30px;padding-top:7px;margin-bottom:15px;height: 30px;">
	
	<#assign  orderMap=b_method.upDataOneOutNull("oc_orderinfo","order_code,order_status,seller_code,small_seller_code,order_type,order_source,out_order_code","","order_code=:order_code","order_code",(a_order_code?default("")))/> 
	<#if orderMap??>
      ...
      <#if orderMap['order_status']='4497153900010002' &&orderMap['seller_code']='SI2003'>
      		...
      		<#if plusServiceSeller.isKJSeller(orderMap['small_seller_code'])>
      			...
      			<input zapweb_attr_operate_id="c21951e56036111186fd005056925439" class="btn btn-small btn-primary" onclick="zapadmin.window_url('../show/page_add_v_oc_return_money_remark?zw_f_order_code=${a_order_code?default("")}')" type="button" value="取消发货">
      		...
      		<#else>
      			...
      			<#else>
      				<#if orderCollageStatusCode!='449748300001'><#-- 拼团中的订单不展示取消发货 -->
      		...
      		<#if orderCollageStatusCode='449748300001'><#-- 拼团中订单显示此按钮 -->
      			<input class="btn btn-small btn-primary" zapweb_attr_operate_id="96c71ea2336545fb9ce3199f2313b7b3" onclick="zapjs.zw.func_tip(this,'${a_order_code?default("")}','人工补团')" type="button" value="人工补团">
      		</#if>
        </#if>
    </#if>
...
<form class="form-horizontal" method="POST" >
	...
	<div class="control-group">
	<label class="control-label" for="">拼团状态:</label>
	<div class="controls">
		...
		<#list a_order_detail.getPageData() as e_list>
			<tr>
	      	...
	      	<td>
				...
				<#if orderMap['order_status'] == "4497153900010002">
					&nbsp;&nbsp;&nbsp;&nbsp;
					<#assign checkRes=orderEditService.checkOrderSkuEditLimit(a_order_code,e_list[1])>
					<#if checkRes.resultCode == 1>
					<input class="btn btn-small btn-primary" onclick="showSkuPage('${e_list[1]}')" type="button" value="修改">
					<#else>
					<input class="btn btn-small disabled" onclick="zapjs.f.message('${checkRes.resultMessage}')" type="button" value="修改">
					</#if>
				</#if>	
			</td>	
	      	</tr>
...
<#if orderMap['order_status'] == "4497153900010002">
	&nbsp;&nbsp;&nbsp;&nbsp;
	<#assign checkRes=orderEditService.checkOrderEditLimit(a_order_code)>
	<#if checkRes.resultCode == 1>
	<input class="btn btn-small pull-right btn-primary" onclick="showAddressPage()" type="button" value="修改收货地址">
	<#else>
	<input class="btn btn-small pull-right disabled" onclick="zapjs.f.message('${checkRes.resultMessage}')" type="button" value="修改收货地址">
	</#if>
</#if>
</div>
...
function showAddressPage(){
	zapjs.f.window_box({
		id : 'edit',
		content : '<iframe src="../page/page_edit_order_address?orderCode=${a_order_code}" frameborder="0" style="width:100%;height:400px;"></iframe>',
		width : '750',
		height : '450'
	});
}

function showSkuPage(skuCode){
	zapjs.f.window_box({
		id : 'edit',
		content : '<iframe src="../page/page_edit_order_sku?orderCode=${a_order_code}&skuCode='+skuCode+'" frameborder="0" style="width:100%;height:300px;"></iframe>',
		width : '550',
		height : '350'
	});
}
