...
<#macro m_zapmacro_common_field_select   e_field    e_page    e_text_select="">
	<@m_zapmacro_common_field_start text=e_field.getFieldNote() for=e_field.getPageFieldName() />
	        <#if e_field.getPageFieldName()="zw_f_out_channel_id">
...
<script>
    $(function(){ 
		...
		$("#zw_f_isfx_show").change(function(e){
			var val = $(this).children('option:selected').val();
			if(val == '449748600001'){  // 否
				$("#zw_f_default_flag").parent().parent().show();
			}else if(val == '449748600002'){   // 是
				$("#zw_f_default_flag").parent().parent().hide();
			} 
		});
    });
    ...
    function init(){
    	...
    	if($('#zw_f_flag_0').prop('checked')){  // 如果 开启猜你喜欢
    		$("#zw_f_class_type").parent().parent().show();
    		// 指定类别
    		if($("#zw_f_class_type").val() == '4497480100040002'){  // 按分类设置
    			$("#zw_f_classify").parent().parent().show();
    			$("#zw_f_key_word").parent().parent().hide();  
    		}else if($("#zw_f_class_type").val() == '4497480100040003'){  // 按关键字
    			$("#zw_f_key_word").parent().parent().show();
    	...
    	if($("#zw_f_isfx_show").val() == '449748600002'){
    		$("#zw_f_default_flag").parent().parent().hide();
    	}
    }
    ...
    function initHidden(){
    	...
    	// 开始初始化 分类设置
		var classifyValue = $("#zw_f_classify").val();
		...
		api_call('com_cmall_familyhas_api_ApiForSellerCategory', opt , function(e){
			...
			$("#zw_f_classify").find("option[value='" + classifyValue + "']").attr("selected",true);
		}); 
    ...
    function defaultHidden(){
    	$("#zw_f_class_type").parent().parent().hide();
    	$("#zw_f_classify").parent().parent().hide();  // 分类设置
    	$("#zw_f_key_word").parent().parent().hide();
    	$("#zw_f_classify option:first").prop("selected", 'selected'); 
