<script>
require(['cfamily/js/advertInfoEdit'],

function()
{
	zapjs.f.ready(function()
		{
			advertisementInfoEdit.init();
		}
	);
}
	  		...
	  		<div class="control-group">
				...
				<div class="controls">
					<select name="zw_f_programa_num" id="zw_f_programa_num">
							<option value="1" <#if  e_field.getPageFieldValue()=='1'> selected="selected" </#if>>1</option>
							<option value="2" <#if  e_field.getPageFieldValue()=='2'> selected="selected" </#if>>2</option>
							<option value="3" <#if  e_field.getPageFieldValue()=='3'> selected="selected" </#if>>3</option>
							<option value="4" <#if  e_field.getPageFieldValue()=='4'> selected="selected" </#if>>4</option>
		      		</select>
				</div>
			</div>
	  		<#else>
