<#assign lService=b_method.upClass("com.cmall.familyhas.service.WeChatBoundService")>
<#assign resultMap=lService.getWeChatBoundActivity()>
...
<#--活动设置页面-->
<script>
require(['cfamily/js/wechatBoundChart'],
function()
{
	zapjs.f.ready(function()
		{
			wechatBoundChart.init(${resultMap});
		}
	);
});
</script>
...
<#macro m_zapmacro_common_auto_operate     e_list_operates  e_area_type>
	<div class="control-group">
    	<div class="controls">
    		...
    		<input type="button" class="btn  btn-success"  onclick="wechatBoundChart.bachCustIdImport()"   value="导入客代号">
    		<input type="button" class="btn  btn-success"  onclick="wechatBoundChart.boundCounpActivity()"   value="绑定优惠券活动">
	...
	<thead>
	   <tr>
	      <#list e_pagedata.getPageHead() as e_list>
	        ...
	        <th colspan="4" width="10%">	解绑微信</th>
	        <th colspan="4" width="10%">	绑定微信</th>
	        <th colspan="4" width="10%">	发放优惠券</th>
	    </tr>
  	</thead>
		...
		<#list e_pagedata.getPageData() as e_list>
			<tr>
	      	...
	      	<td colspan="4" style="vertical-align:middle; text-align:center;">
				    <#if (e_list[statusIndex] = '是')>
					   <input  onclick="wechatBoundChart.changeWeChatBoundStatus('${e_list[0]}','0')" type="button" class="btn btn-small" value="解绑微信">	             
				   </#if>
			</td>
			
			<td colspan="4" style="vertical-align:middle; text-align:center;">
			    <#if (e_list[statusIndex] == '否')>
					   <input  onclick="wechatBoundChart.changeWeChatBoundStatus('${e_list[0]}','1')" type="button" class="btn btn-small" value="绑定微信">	             
				</#if>
		   </td>
		   
		   <td colspan="4" style="vertical-align:middle; text-align:center;">
			    <#if (e_list[statusIndex] == '是')>
			     <input  onclick="wechatBoundChart.issueCoupon('${e_list[1]}')" type="button" class="btn btn-small" value="发放优惠券">
				</#if>
		   </td>
	      	</tr>
	 	</#list>
