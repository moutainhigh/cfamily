...
<div class="control-group">
		<#if column_type='4497471600010031'>
		<div class="controls">
			<a class="btn  btn-success" href="../resources/cfamily/product_Import_template_for_oneColumnBigPic.xls">下载模板</a>
		</div>
		<#else>
		<div class="controls">
			<a class="btn  btn-success" href="../resources/cfamily/product_Import_template.xls">下载模板</a>
		</div>
		</#if> 
	</div>
...
function up_urlparam(key,sUrl) {
 ...
 function importSubmit() {
    ...
    var column_code = unescape(up_urlparam("column_code",''));
    var column_type = unescape(up_urlparam("column_type",''));

 	var obj = {};
	obj.upload_show = $('#zw_f_upload_show').val();
	obj.column_code = column_code;
	obj.column_type = column_type;
	zapjs.zw.api_call('com_cmall_familyhas_api_ApiImportColumnContent',obj,function(result) {
