...
<#macro m_zapmacro_common_auto_inquire e_page>
	<#list e_page.upInquireData() as e>
		...
		<#elseif e.getQueryTypeAid()=="104009001">
	  	...
	  	<#if e.getColumnName()=="bear_dept">
			<div class="control-group">
				<label class="control-label" for="zw_f_coupon_source">优惠券来源</label>
				<div class="controls">
					<select id="zw_f_coupon_source" name="zw_f_coupon_source">
						<option value="">请选择</option>
						<#if RequestParameters['zw_f_coupon_source']?default('')=='ld'>
							<option value="ld" selected>LD</option>
						<#else>
							<option value="ld">LD</option>
						</#if>
						<#if RequestParameters['zw_f_coupon_source']?default('')=='hjy'>
							<option value="hjy" selected>惠家有</option>
						<#else>
							<option value="hjy">惠家有</option>
						</#if>		
...
<table  class="table  table-condensed table-striped table-bordered table-hover">
	<thead>
	   <tr>
	        <#list e_pagedata.getPageHead() as e_list>
	        	<#if (e_list_index <=11)&&(e_list_index > 0)>
		      	 <th>
		      	 	${e_list}
		      	 </th>
		      	 </#if>
	...
	<tbody>
		<#list e_pagedata.getPageData() as e_list>
			<tr>
	  		 <#list e_list as e>
	  		 <#if (e_index <=11)&&(e_index > 0)>
	      		<td>
	      			 <#if (e_index = 6) && (e = '否')>
	      			 	未发布
	      			 <#elseif (e_index = 6) && (e = '是')>
	      			 	已发布
	      			 <#else>
		      			${e?default("")}
	      			 </#if>
	      	...
	      	<td colspan="4" style="vertical-align:middle; text-align:center;">
				<#if (e_list[6]=='否' )>
					<a class="btn btn-link" target="_blank" href="page_edit_v_coupon_oc_activity_new3?zw_f_uid=${e_list[0]}">修改</a>
					<#if e_list[10]!='兑换码兑换'&&e_list[10]!='系统返利'>
						<a class="btn btn-link" target="_blank" href=" page_bookEdit_v_coupon_oc_activity_new3?zw_f_uid=${e_list[0]}&zw_f_activity_code=${e_list[1]}">优惠券类型管理</a>
					</#if>
				...
				<a class="btn btn-link" target="_blank" href="page_book_v_coupon_oc_activity_new3?zw_f_uid=${e_list[0]}&zw_f_activity_code=${e_list[1]}">查看</a>
				
				<#assign couponNum=couponsService.getCouponTypeNum( '${e_list[1]}')>
				<#if 0 < couponNum && e_list[10]!='兑换码兑换'&&e_list[10]!='系统返利'>
					<a class="btn btn-link" target="_blank" href="page_add_v_oc_coupon_cdkey?zw_f_activity_code=${e_list[1]}">生成优惠码</a>
				</#if>
				
				<#if e_list[10]=='系统发放' && e_list[6]='是'>
					<#if 0 < couponNum && e_list[7]!='ld'>
						<a class="btn btn-link" target="_blank" href="page_add_v_oc_coupon_check?zw_f_activity_code=${e_list[1]}">发放到指定账户</a>
					</#if>
				</#if>
				<#if e_list[10]=='兑换码兑换' && e_list[6]='是'>
					<a class="btn btn-link" target="_blank" href="page_add_v_oc_coupon_redeem?zw_f_activity_code=${e_list[1]}">发放到指定账户</a>
				</#if>
			</td>
	      	</tr>
	 	</#list>
