...
<#macro m_zapmacro_common_auto_list e_pagedata e_page>
	...
	<#if e_pagedata??>
			...
			<div class="control-group">
				...
				<div class="controls">
					<input type="text" id="zw_f_code" name="zw_f_code"  zapweb_attr_regex_id="469923180002"  value="">
				</div>
				</div>
					...
					<#if e_field.getPageFieldName() = "zw_f_account_day">
						<select name="${e_field.getPageFieldName()}" id="${e_field.getPageFieldName()}">
					      			...
					      			<#list e_page.upDataSource(e_field) as e_key>
										...
										<option value="${e_key.getV()}" <#if  e_field.getPageFieldValue()==e_key.getV()> </#if>>${e_key.getK()}</option>
									</#list>
					      		</select>
					</#if>
