...
<div class="zab_info_page_title  w_clear">
<span>栏目内容列表</span><a  class="btn btn-small" href="page_add_v_apphome_extrage?zw_f_channel_uid=${RequestParameters['zw_f_uid']}" style="height:20px;margin-left:10px">新增栏目内容</a>
...
<#assign channelDetails=b_method.upControlPage("page_chart_v_fh_apphome_channel_extracharge","zw_f_channel_uid=" + channel_uid + "&" + "zw_p_size=-1").upChartData()>
...
$(function(){
    ...
    for(var i = 0 ; i < trArr.length; i ++){
    	...
    	if(isRelease == 1 && status == 2){
    		$(arr[9].children[0]).attr("type","hidden");
    		$(arr[9].children[0]).attr("onclick","");
    	}else{
    		$(arr[9].children[0]).val("删除");
    	}
