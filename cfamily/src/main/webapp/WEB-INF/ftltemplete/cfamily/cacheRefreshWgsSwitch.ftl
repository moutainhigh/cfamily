...
<#assign vOpt=cService.getCgroupMoneySwitch("withdraw")>
<#assign operateValue="">
...
<#macro m_zapmacro_common_auto_list e_pagedata   e_page>
	<div class="control-group">
		<label class="control-label" for="zw_f_operate_value">是否可用</label>
		<div class="controls">
	  		<select name="zw_f_operate_value" id="zw_f_operate_value">
	  				<option value="">请选择</option>
					<option value="0"  <#if "0"==vOpt> selected="selected" </#if>>不可用</option>
					<option value="1"  <#if "1"==vOpt> selected="selected" </#if>>可用</option>
	  		</select>
		</div>
	</div>
</#macro>