...
<form class="form-horizontal" method="POST" >
	<input type="hidden" name="zw_f_template_type" value="${t_type}" id="zw_f_template_type"/>
...
<#macro m_zapmacro_common_field_text e_field>
	<@m_zapmacro_common_field_start text=e_field.getFieldNote() for=e_field.getPageFieldName() />
		...
		<#if e_field.getPageFieldName()=="zw_f_title">
			<span style="line-height:30px;color:#FF0000;">（建议6个字以内）</span>
		</#if>
	<@m_zapmacro_common_field_end />
</#macro>
...
<#macro m_zapmacro_common_field_textarea e_field>
	<@m_zapmacro_common_field_start text=e_field.getFieldNote() for=e_field.getPageFieldName() />
		...
		<#if e_field.getPageFieldName()=="zw_f_describes">
			<span style="line-height:30px;color:#FF0000;">（建议10个字以内）</span>
		</#if> 
	<@m_zapmacro_common_field_end />
</#macro>
...
<#macro m_zapmacro_common_field_start text="" for="">
	...
	<div class="control-group" style="position: relative;">
		<label class="control-label" for="${for}">
			...
			<#if (for=="zw_f_url")>
				<span class="w_regex_need">*</span>
				<span style="position: absolute;left: 430px;line-height:30px;color:#BFBCBC;">若填写的视频连接是ccvid,请以填写ccvid的编号,例如：3F394F2ACC69682D9C33DC5901307465</span>
				<span style="position: absolute;left: 430px;line-height:30px;color:#BFBCBC;top: 20px;">若填写的是页面链接，请填写正确的页面连接</span>
			</#if>
			...
			<#if (for=="zw_f_preferential_desc")>
				<span class="w_regex_need">*</span>
				<span style="position: absolute;left: 430px;line-height:30px;color:#FF0000;">（优惠描述最多4个字！）</span>
			</#if>
...
<#macro m_zapmacro_common_operate_button  e_operate  e_style_css>
	...
	<input type="button" class="${e_style_css}" zapweb_attr_operate_id="${e_operate.getOperateUid()}"  onclick="editCommodity.submit(this);"  value="${e_operate.getOperateName()}" />
</#macro>
...
<#macro m_zapmacro_common_field_span e_field>
		...
		<div class="control-group" id="selectPcCtrol">
			...
			<div class="controls">
				<div>
					...
					<input class="btn btn-small" type="button" value="选择商品" onclick="editCommodity.show_windows()">
					<#else>
					<script type="text/javascript">
	...
	<#if e_field.getPageFieldName()="zw_f_commodity_number" || e_field.getPageFieldName()="zw_f_template_number" || e_field.getPageFieldName()="zw_f_good_number">
		<@m_zapmacro_common_field_start text=e_field.getFieldNote() for=e_field.getPageFieldName() />
			<input type="text"  id="${e_field.getPageFieldName()}" name="${e_field.getPageFieldName()}" value="${e_field.getPageFieldValue()}"  readonly="readonly" />
		<@m_zapmacro_common_field_end />
		
	<#elseif e_field.getPageFieldName()="zw_f_sub_template_number">
		
		
		<@m_zapmacro_common_field_start text=e_field.getFieldNote() for=e_field.getPageFieldName() />
			<div class="w_left">
				<input id="zw_f_selectedTemplate_id" class="btn" type="button" onclick="pageTemplete_single.show_box('zw_f_sub_template_number')" value="选择">
			</div>
			<input type="text"  id="${e_field.getPageFieldName()}" name="${e_field.getPageFieldName()}" value="${e_field.getPageFieldValue()}"  readonly="readonly" />
		<@m_zapmacro_common_field_end />
		<script>
...
<#macro m_zapmacro_common_field_span_ymmb e_pagedata e_page>
		<@m_zapmacro_common_field_hidden e_pagedata[0]/>
		...
		<@m_zapmacro_common_field_start text=e_pagedata[21].getFieldNote() for="${e_pagedata[21].getPageFieldName()}" />
			<input type="text" id="${e_pagedata[21].getPageFieldName()}" name="${e_pagedata[21].getPageFieldName()}" value="${e_pagedata[21].getPageFieldValue()}" />
		<@m_zapmacro_common_field_end />
		<@m_zapmacro_common_field_start text=e_pagedata[22].getFieldNote() for="${e_pagedata[22].getPageFieldName()}" />
			<input type="text" id="${e_pagedata[22].getPageFieldName()}" name="${e_pagedata[22].getPageFieldName()}" value="${e_pagedata[22].getPageFieldValue()}" />
		<@m_zapmacro_common_field_end />
		<@m_zapmacro_common_field_start text=e_pagedata[11].getFieldNote() for="${e_pagedata[11].getPageFieldName()}" />
			<input type="text" autocomplete="off" onClick="laydate({istime:true,format:'YYYY-MM-DD hh:mm:ss'})" id="${e_pagedata[11].getPageFieldName()}" name="${e_pagedata[11].getPageFieldName()}" value="${e_pagedata[11].getPageFieldValue()}">
		<@m_zapmacro_common_field_end />
		<@m_zapmacro_common_field_start text=e_pagedata[12].getFieldNote() for="${e_pagedata[12].getPageFieldName()}" />
			<input type="text" autocomplete="off" onClick="laydate({istime:true,format:'YYYY-MM-DD hh:mm:ss'})" id="${e_pagedata[12].getPageFieldName()}" name="${e_pagedata[12].getPageFieldName()}" value="${e_pagedata[12].getPageFieldValue()}">
		<@m_zapmacro_common_field_end />
		<@m_zapmacro_common_field_start text=e_pagedata[26].getFieldNote() for="${e_pagedata[26].getPageFieldName()}" />
			...
			<ul id="zw_f_sub_template_number_show_ul" style="list-style: none;">
				...
				<#if (e_pagedata[26].getPageFieldValue()?length > 0) >
					<span>${e_pagedata[26].getPageFieldValue() }</span>
					<#list e_pagedata[26].getPageFieldValue()?split(",") as templateNum>
						<#if (templateNum?length > 0) >
							<li id="select_num_${templateNum }" class="control-upload-list-li"><div><div class="w_left w_p_10"><span>${templateNum }</span><a href="javascript:void(0);" onclick="pageTemplete_single.change_index(this,'up')">上移</a><a href="javascript:void(0);" onclick="pageTemplete_single.change_index(this,'down')">下移</a><a href="javascript:void(0);" onclick="pageTemplete_single.change_index(this,'delete')">删除</a></div><div class="w_clear"></div></div></li>
						</#if>
					</#list>
				</#if>
		...
		<@m_zapmacro_common_html_script "zapjs.f.require(['lib/datepicker/WdatePicker'],function(a){});" />
		<script>
			zapjs.f.require(['zapadmin/js/pageTemplete_single'],function(a){pageTemplete_single.init({"text":"","source":"page_chart_v_fh_pageTemple_single","id":"zw_f_sub_template_number","value":"","seller_code":"SI2003","max":"100","single":false});});
		</script>
</#macro>
...
<#macro m_zapmacro_common_field_span_djsmb e_pagedata e_page>
        <@m_zapmacro_common_field_hidden e_pagedata[0]/>
		<@m_zapmacro_common_field_start text=e_pagedata[27].getFieldNote() for="${e_pagedata[27].getPageFieldName()}" />
			<input type="text" id="${e_pagedata[27].getPageFieldName()}" name="${e_pagedata[27].getPageFieldName()}" value="${e_pagedata[27].getPageFieldValue()}"  readonly="readonly"/>
		<@m_zapmacro_common_field_end />
		<@m_zapmacro_common_field_start text=e_pagedata[20].getFieldNote() for="${e_pagedata[20].getPageFieldName()}" />
			<input type="text" id="${e_pagedata[20].getPageFieldName()}" name="${e_pagedata[20].getPageFieldName()}" value="${e_pagedata[20].getPageFieldValue()}" />
		<@m_zapmacro_common_field_end />
...
<script>
require(['cfamily/js/editCommodity'],function(a){a.init(${t_type})});
</script>
<script type="text/javascript">
