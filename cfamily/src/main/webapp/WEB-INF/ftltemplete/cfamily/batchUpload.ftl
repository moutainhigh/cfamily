...
<#assign callback = b_page.getReqMap()["callback"]! >
...
function customSuccess(o){
	if(o.resultCode != 1 || callback == '') {
		zapjs.zw.func_success(o)
		return;
	}
	
	eval(callback+'('+JSON.stringify(o)+')' );
	parent.zapjs.f.window_close('importProductShowBox');
}
</script>