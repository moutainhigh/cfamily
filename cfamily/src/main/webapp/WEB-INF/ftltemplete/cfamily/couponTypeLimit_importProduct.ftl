...
<div class="control-group">
	    <div class="controls">
			<a class="btn  btn-success" href="../resources/cfamily/couponTypeProductImport.xls">下载模板</a>
		</div>
	</div>
...
<script type="text/javascript">
 function importSubmit() {
	...
	zapjs.zw.api_call('com_cmall_familyhas_api_ApiCouponTypeLimitImportProduct',obj,function(result) {
			...
			if(result.resultMessage!='') {
				...
				if(success.length > 0) {
					for(var i = 0; i < success.length; i++) {
						...
						if(parent.couponTypeLimit_product_select.checkProductSame(productInfo[0])) {
							//重复
							if(same.length > 0) {
								same += "," + productInfo[0];
							} else {
							...
							//不重复
							parent.couponTypeLimit_product_select.addProductRow(productInfo[0], productInfo[1]);
						}
					}
				}
				...
				parent.couponTypeLimit_product_select.close_import_product_win(msg);			
			}
		} else {
			zapadmin.model_message('调用远程接口com_cmall_familyhas_api_ApiCouponTypeLimitImportProduct导入商品失败!');
		}
	});
