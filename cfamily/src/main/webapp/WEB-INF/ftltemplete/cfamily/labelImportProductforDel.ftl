 ...
 function importSubmit() {
	...
	zapjs.zw.api_call('com_cmall_familyhas_api_ApiLabelImportProduct',obj,function(result) {
		...
		if(result.resultCode==1){
			...
			if(result.resultMessage!='') {
				...
				if(success.length > 0) {
					for(var i = 0; i < success.length; i++) {
						...
						if(!parent.label_product_select.checkProductifHas(productInfo[0])) {
							//不存在
							if(notFound.length > 0) {
								notFound += "," + productInfo[0];
							} else {
								notFound += productInfo[0] + ",";
							}
							continue;
							...
							//不重复
							parent.label_product_select.delProductRow(productInfo[0]);
						}
					}
				}
				
				if (notFound.length > 0 && notFound.substring(notFound.length-",".length)==",") {
			...
			zapadmin.model_message('调用远程接口com_cmall_familyhas_api_ApiLabelImportProduct批量删除商品失败!');
		}
	});
